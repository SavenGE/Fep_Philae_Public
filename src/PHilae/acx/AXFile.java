...
public class AXFile implements Serializable {
    ...
    public String getFile(String directory, String prefix, String suffix) {
        String fileName = null;
        for (File file : getFiles(directory)) {
            String name = file.getName().toLowerCase().trim();
            if (name.equalsIgnoreCase(prefix.trim() + suffix.trim()) || name.equalsIgnoreCase(prefix.trim() + "." + suffix.trim())) {
                return file.getName();
            } else if (name.startsWith(prefix.toLowerCase().trim()) && name.endsWith(suffix.toLowerCase().trim())) {
    ...
    public void copy(File srcFile, String dstDir) throws Exception {
        ...
        writeToFile(getBytes(srcFile.getAbsolutePath()), dstDir, tempFileName);
        if (!"".equals(fExt)) {
    ...
    public boolean clearDirectory(final File directory) {
        boolean success = true;
        for (final File file : (directory.isDirectory() ? directory.listFiles() : new File[0])) {
            try {
                deleteFile(file);
    ...
    public boolean clearDirectory(final String fileName) {
        return clearDirectory(new File(fileName));
    }

    public boolean deleteFile(final File file) {
        return file.isDirectory() ? deleteDirectory(file) : !file.exists() || file.delete();
    }
