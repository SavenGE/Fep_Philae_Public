...
public class AcroFields {
     ...
     * <
     * PRE>
     * PdfReader pdf = new PdfReader("input.pdf"); PdfStamper stp = new
     * PdfStamper(pdf, new FileOutputStream("output.pdf")); AcroFields af =
     * stp.getAcroFields(); af.setListOption("ComboBox", new String[]{"a", "b",
     * "c"}, new String[]{"first", "second", "third"}); af.setField("ComboBox",
     * "b"); stp.close();
     * </PRE>
     *
     * @param fieldName the field name
     ...
     * "bgcolor", BaseColor.BLUE, null); This won't have any effect, unless you
     * use regenerateField("f") after changing the property.
     *
     * @param name the fully qualified field name or the partial name in the
     * case of XFA forms
     * @throws IOException on error
     * @throws DocumentException on error
    ...
    /**
     * Sets the rich value for the given field. See
     ...
     * further details. Note that iText doesn't create an appearance for Rich
     * Text fields. So you either need to use XML Worker to create an appearance
     * (/N entry in the /AP dictionary), or you need to use
     * setGenerateAppearances(false) to tell the viewer that iText didn't create
     * any appearances.
     *
     ...
     * <CODE>null</CODE> the <CODE>value</CODE> parameter will be used
     * @return <CODE>true</CODE> if the field was found and changed,
     * <CODE>false</CODE> otherwise
     * @throws IOException on error
     * @throws DocumentException on error
     ...
     *
     * @param name the field name
     * @return true if the field was signed, false if the field was not signed
     * or not found
     * @since 5.0.5
     */
     ...
     * PdfReader("my_signed_doc.pdf"); AcroFields af = reader.getAcroFields();
     * ArrayList names = af.getSignatureNames(); for (int k = 0; k &lt;
     * names.size(); ++k) { String name = (String)names.get(k);
     * System.out.println("Signature name: " + name);
     * System.out.println("Signature covers whole document: " +
     * af.signatureCoversWholeDocument(name)); PdfPKCS7 pk =
     * af.verifySignature(name); Calendar cal = pk.getSignDate(); Certificate
     * pkc[] = pk.getCertificates(); System.out.println("Subject: " +
     * PdfPKCS7.getSubjectFields(pk.getSigningCertificate()));
     * System.out.println("Document modified: " + !pk.verify()); Object fails[]
     * = PdfPKCS7.verifyCertificates(pkc, kall, null, cal); if (fails == null)
     * System.out.println("Certificates verified against the KeyStore"); else
     ...
     * <
     * pre>
     * String pdfFile = ...;// the pdf file used as template ArrayList xfdfFiles
     * = ...;// the xfdf file names ArrayList pdfOutFiles = ...;// the output
     * file names, one for each element in xpdfFiles HashMap cache = new
     * HashMap();// the appearances cache PdfReader originalReader = new
     * PdfReader(pdfFile); for (int k = 0; k &lt; xfdfFiles.size(); ++k) {
     * PdfReader reader = new PdfReader(originalReader); XfdfReader xfdf = new
     * XfdfReader((String)xfdfFiles.get(k)); PdfStamper stp = new
     * PdfStamper(reader, new FileOutputStream((String)pdfOutFiles.get(k)));
     * AcroFields af = stp.getAcroFields(); af.setFieldCache(cache);
     * af.setFields(xfdf); stp.close(); }
     * </pre>
     *
