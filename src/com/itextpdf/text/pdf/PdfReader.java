...
public class PdfReader implements PdfViewerPreferences {
     ...
     *
     * @param obj the object. If it's an indirect reference it will be
     * eliminated
...
//                            tokens.throwError(MessageLocalization.getComposedMessage("file.position.0.cross.reference.entry.in.this.xref.subsection"));
     ...
     *
     * @param in the input data
     * @param strict <CODE>true</CODE> to read a correct stream.
     * <CODE>false</CODE> to try to read a corrupted stream
     * @return the decoded data
     */
...
//            throw new RuntimeException(MessageLocalization.getComposedMessage("illegal.length.in.ascii85decode"));
     ...
     *
     * @param pageNum 1-based page number from which to retrieve the resource
     * dictionary
    ...
    /**
     * Decode a byte[] applying the filters specified in the provided dictionary
     * using the provided filter handlers.
     *
     * @param b the bytes to decode
     * @param streamDictionary the dictionary that contains filter information
     * @param filterHandlers the map used to look up a handler for each type of
     * filter
     * @return the decoded bytes
     * @throws IOException if there are any problems decoding the bytes
     * @since 5.0.4
     ...
     * the name and the value is the destinations array.
     *
     * @param	keepNames	true if you want the keys to be real PdfNames instead of
     * Strings
     * @return gets all the named destinations
     * @since	2.1.6
     */
     ...
     *
     * @param	obj	an annotation that needs to be screened for links to external
     * named destinations.
     ...
     *
     * @param ranges the comma separated ranges as described in
     * {@link SequenceList}
     */
     ...
     * password or if it's not encrypted, <CODE>false</CODE> if the document was
    ...
    /**
     * @return byte array of computed user password, or null if not encrypted or
     * no ownerPassword is used.
     */
