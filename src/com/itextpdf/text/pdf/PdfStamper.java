...
public class PdfStamper
     ...
     * @param reader the original document. It cannot be reused
     * @param os the output stream
     * @param pdfVersion the new pdf version or '\0' to keep the same version as
     ...
     * revision. This is only useful for multiple signatures as nothing is
     * gained in speed or memory
     * @throws DocumentException on error
     * @throws IOException on error
     */
     ...
     *
     * @param pageNumber the page number position where the new page will be
     * inserted
     ...
     *
     * @param rotateContents <CODE>true</CODE> to set auto-rotation,
     ...
     * <CODE>false</CODE> (the default) to keep the FreeText annotations as
     * active content.
     */
     ...
     *
     * @param actionType the action type. It can be
     * <CODE>PdfWriter.PAGE_OPEN</CODE> or <CODE>PdfWriter.PAGE_CLOSE</CODE>
     * @param action the action to perform
     * @param page the page where the action will be applied. The first page is
     * 1
     ...
     *
     * @param seconds the number of seconds to display the page. A negative
     * value removes the entry
     * @param page the page where the duration will be applied. The first page
     * is 1
     */
     ...
     * <
     * pre>
     * KeyStore ks = KeyStore.getInstance("pkcs12"); ks.load(new
     * FileInputStream("my_private_key.pfx"), "my_password".toCharArray());
     * String alias = (String)ks.aliases().nextElement(); PrivateKey key =
     * (PrivateKey)ks.getKey(alias, "my_password".toCharArray()); Certificate[]
     * chain = ks.getCertificateChain(alias); PdfReader reader = new
     * PdfReader("original.pdf"); FileOutputStream fout = new
     * FileOutputStream("signed.pdf"); PdfStamper stp =
     * PdfStamper.createSignature(reader, fout, '\0', new File("/temp"), true);
     * PdfSignatureAppearance sap = stp.getSignatureAppearance();
     * sap.setCrypto(key, chain, null, PdfSignatureAppearance.WINCER_SIGNED);
     * sap.setReason("I'm the author"); sap.setLocation("Lisbon"); // comment
     * next line to have an invisible signature sap.setVisibleSignature(new
     * Rectangle(100, 100, 200, 200), 1, null); stp.close();
     * </pre>
     *
     ...
     * temporary file will be created there. If it's a file it will be used
     * directly. The file will be deleted on exit unless <CODE>os</CODE> is
     * null. In that case the document can be retrieved directly from the
     * temporary file. If it's <CODE>null</CODE> no temporary file will be
     * created and memory will be used
     * @param append if <CODE>true</CODE> the signature and all the other
     ...
     * PdfReader("original.pdf"); FileOutputStream fout = new
     * FileOutputStream("signed.pdf"); PdfStamper stp =
     * PdfStamper.createSignature(reader, fout, '\0'); PdfSignatureAppearance
     * sap = stp.getSignatureAppearance(); sap.setCrypto(key, chain, null,
     * PdfSignatureAppearance.WINCER_SIGNED); sap.setReason("I'm the author");
     * sap.setLocation("Lisbon"); // comment next line to have an invisible
     * signature sap.setVisibleSignature(new Rectangle(100, 100, 200, 200), 1,
     * null); stp.close();
     * </pre>
     *
     * @param reader the original document
     ...
     * PdfReader("original.pdf"); FileOutputStream fout = new
     * FileOutputStream("signed.pdf"); PdfStamper stp =
     * PdfStamper.createSignature(reader, fout, '\0', new File("/temp"));
     * PdfSignatureAppearance sap = stp.getSignatureAppearance();
     * sap.setCrypto(key, chain, null, PdfSignatureAppearance.WINCER_SIGNED);
     * sap.setReason("I'm the author"); sap.setLocation("Lisbon"); // comment
     * next line to have an invisible signature sap.setVisibleSignature(new
