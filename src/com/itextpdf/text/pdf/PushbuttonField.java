 ...
 * document.open(); PdfContentByte cb = writer.getDirectContent(); Image img =
 * Image.getInstance("image.png"); PushbuttonField bt = new
 * PushbuttonField(writer, new Rectangle(100, 100, 200, 200), "Button1");
 * bt.setText("My Caption"); bt.setFontSize(0); bt.setImage(img);
 * bt.setLayout(PushbuttonField.LAYOUT_ICON_TOP_LABEL_BOTTOM);
 * bt.setBackgroundColor(Color.cyan);
 * bt.setBorderStyle(PdfBorderDictionary.STYLE_SOLID);
 * bt.setBorderColor(Color.red); bt.setBorderWidth(3); PdfFormField ff =
 * bt.getField(); PdfAction ac =
 * PdfAction.createSubmitForm("http://www.submit-site.com", null, 0);
...
public class PushbuttonField extends BaseField {
     ...
     * @param fieldName the field name. If <CODE>null</CODE> only the widget
     * keys will be included in the field allowing it to be used as a kid field.
     */
     ...
     *
     * @param iconVerticalAdjustment a number between 0 and 1 indicating the
     * fraction of leftover space to allocate at the bottom of the icon
     */
     ...
     *
     * @param iconHorizontalAdjustment a number between 0 and 1 indicating the
     * fraction of leftover space to allocate at the left of the icon
     */
     ...
     *
     * @param iconFitToBounds if <CODE>true</CODE> the icon will be scaled to
     * fit fully within the bounds of the annotation, if <CODE>false</CODE> the
