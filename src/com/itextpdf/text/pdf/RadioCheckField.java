 ...
 * <
 * PRE>
 * Document document = new Document(PageSize.A4, 50, 50, 50, 50); PdfWriter
 ...
 * RadioCheckField bt = new RadioCheckField(writer, new Rectangle(100, 100, 200,
 * 200), "radio", "v1"); bt.setCheckType(RadioCheckField.TYPE_CIRCLE);
 * bt.setBackgroundColor(Color.cyan);
 * bt.setBorderStyle(PdfBorderDictionary.STYLE_SOLID);
 * bt.setBorderColor(Color.red); bt.setTextColor(Color.yellow);
 * bt.setBorderWidth(BaseField.BORDER_WIDTH_THICK); bt.setChecked(false);
 * PdfFormField f1 = bt.getRadioField(); bt.setOnValue("v2");
 * bt.setChecked(true); bt.setBox(new Rectangle(100, 300, 200, 400));
 * PdfFormField f2 = bt.getRadioField(); bt.setChecked(false); PdfFormField top
 * = bt.getRadioGroup(true, false); bt.setOnValue("v3"); bt.setBox(new
 * Rectangle(100, 500, 200, 600)); PdfFormField f3 = bt.getRadioField();
 * top.addKid(f1); top.addKid(f2); top.addKid(f3); writer.addAnnotation(top); bt
 * = new RadioCheckField(writer, new Rectangle(300, 300, 400, 400), "check1",
 * "Yes"); bt.setCheckType(RadioCheckField.TYPE_CHECK);
 * bt.setBorderWidth(BaseField.BORDER_WIDTH_THIN);
 * bt.setBorderColor(Color.black); bt.setBackgroundColor(Color.white);
 * PdfFormField ck = bt.getCheckField(); writer.addAnnotation(ck);
 * document.close();
 * </PRE>
 *
 * @author Paulo Soares
...
public class RadioCheckField extends BaseField {
     ...
     *
     * @param noToggleToOff if <CODE>true</CODE>, exactly one radio button must
     * be selected at all times; clicking the currently selected button has no
     * effect. If <CODE>false</CODE>, clicking the selected button deselects it,
     * leaving no button selected.
     * @param radiosInUnison if <CODE>true</CODE>, a group of radio buttons
     * within a radio button field that use the same value for the on state will
     * turn on and off in unison; that is if one is checked, they are all
     * checked. If <CODE>false</CODE>, the buttons are mutually exclusive (the
     ...
     *
     * @param isRadio <CODE>true</CODE> to get a radio field, <CODE>false</CODE>
