...
public final class InlineImageUtils {
    ...
    /**
     * Simple class in case users need to differentiate an exception from
     * processing inline images vs other exceptions
     *
     * @since 5.0.4
     */
    ...
    /**
     * Map between key abbreviations allowed in dictionary of inline images and
     * their equivalent image dictionary keys
     */
    ...
    /**
     * Map between value abbreviations allowed in dictionary of inline images
     * for COLORSPACE
     */
    ...
    /**
     * Parses an inline image from the provided content parser. The parser must
     * be positioned immediately following the BI operator in the content
     * stream. The parser will be left with current position immediately
     * following the EI operator that terminates the inline image
     *
     ...
     * @throws InlineImageParseException if parsing of the inline image failed
     * due to issues specific to inline image processing
     */
    ...
    /**
     * Parses the next inline image dictionary from the parser. The parser must
     * be positioned immediately following the EI operator. The parser will be
     * left with position immediately following the whitespace character that
     * follows the ID operator that ends the inline image dictionary.
     *
     * @param ps the parser to extract the embedded image information from
     * @return the dictionary for the inline image, with any abbreviations
     * converted to regular image dictionary keys and values
     ...
     * @return if value is an allowed abbreviation for the key, the expanded
     * value for that abbreviation. Otherwise, value is returned without
     * modification
     */
    ...
    /**
     * @param colorSpaceName the name of the color space. If null, a bi-tonal
     * (black and white) color space is assumed.
    ...
    /**
     * Computes the number of unfiltered bytes that each row of the image will
     * contain. If the number of bytes results in a partial terminating byte,
     * this number is rounded up per the PDF specification
     *
    ...
    /**
     * Parses the samples of the image from the underlying content parser,
     * ignoring all filters. The parser must be positioned immediately after the
     * ID operator that ends the inline image's dictionary. The parser will be
     * left positioned immediately following the EI operator. This is primarily
     * useful if no filters have been applied.
     *
     * @param imageDictionary the dictionary of the inline image
    ...
    /**
     * Parses the samples of the image from the underlying content parser,
     * accounting for filters The parser must be positioned immediately after
     * the ID operator that ends the inline image's dictionary. The parser will
     * be left positioned immediately following the EI operator.
     * <b>Note:</b>This implementation does not actually apply the filters at
     * this time
     *
     * @param imageDictionary the dictionary of the inline image
     * @param ps the content parser
