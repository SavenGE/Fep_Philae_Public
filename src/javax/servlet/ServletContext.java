...
public interface ServletContext {
    ...
    /**
     * The name of the <tt>ServletContext</tt> attribute which stores the
     * private temporary directory (of type
     * <tt>java.io.File</tt>) provided by the servlet container for the
     * <tt>ServletContext</tt>
     */
    public static final String TEMPDIR = "javax.servlet.context.tempdir";

    /**
     * The name of the <code>ServletContext</code> attribute whose value (of
     * type <code>java.util.List&lt;java.lang.String&gt;</code>) contains the
     * list of names of JAR files in <code>WEB-INF/lib</code> ordered by their
     * web fragment names (with possible exclusions if
     * <code>&lt;absolute-ordering&gt;</code> without any
     * <code>&lt;others/&gt;</code> is being used), or null if no absolute or
     * relative ordering has been specified
     */
     ...
     * request URI. If this context is the “default” context rooted at the base
     * of the Web server’s URL name space, this path will be an empty string.
     * Otherwise, if the context is not rooted at the root of the server’s name
     * space, the path starts with a / character but does not end with a /
     * character.
     *
     * <p>
     * It is possible that a servlet container may match a context by more than
     * one context path. In such cases the
     * {@link javax.servlet.http.HttpServletRequest#getContextPath()} will
     * return the actual context path used by the request and it may differ from
     * the path returned by this method. The context path returned by this
     ...
     * objects from the context. The given path must be begin with
     * <tt>/</tt>, is interpreted relative to the server's document root and is
     * matched against the context roots of other web applications hosted on
     * this container.
     *
     * <p>
     ...
     *
     * <p>
     * The value returned may be different from {@link #getMajorVersion}, which
     * returns the major version of the Servlet specification supported by the
     * Servlet container.
     *
     ...
     *
     * <p>
     * The value returned may be different from {@link #getMinorVersion}, which
     * returns the minor version of the Servlet specification supported by the
     * Servlet container.
     *
     ...
     * deployment descriptor. Common MIME types include <code>text/html</code>
     ...
     * <tt>{"/welcome.html", "/catalog/", "/customer/", "/WEB-INF/"}</tt>, and
     * <tt>getResourcePaths("/catalog/")</tt> would return
     * <tt>{"/catalog/index.html", "/catalog/products.html", "/catalog/offers/",
     * "/catalog/moreOffers/"}</tt>.
     *
     ...
     *
     * <p>
     * The path must begin with a <tt>/</tt> and is interpreted as relative to
     * the current context root, or relative to the <tt>/META-INF/resources</tt>
     * directory of a JAR file inside the web application's
     * <tt>/WEB-INF/lib</tt>
     * directory. This method will first search the document root of the web
     * application for the requested resource, before searching any of the JAR
     * files inside <tt>/WEB-INF/lib</tt>. The order in which the JAR files
     * inside <tt>/WEB-INF/lib</tt>
     ...
     *
     * @exception MalformedURLException if the pathname is not given in the
     * correct form
     */
     ...
     *
     * <p>
     * The pathname must begin with a <tt>/</tt> and is interpreted as relative
     * to the current context root. Use <code>getContext</code> to obtain a
     ...
     *
     * <p>
     * For example, if <tt>path</tt> is equal to <tt>/index.html</tt>, this
     * method will return the absolute file path on the server's filesystem to
     * which a request of the form
     ...
     * <tt>/WEB-INF/lib</tt> directory must be considered only if the container
     * has unpacked them from their containing JAR file, in which case the path
     * to the unpacked location must be returned.
     *
     * <p>
     * This method returns <code>null</code> if the servlet container is unable
     * to translate the given
     * <i>virtual</i> path to a
     * <i>real</i> path.
     *
     * @param path the <i>virtual</i> path to be translated to a
     * <i>real</i> path
     *
     * @return the <i>real</i> path, or <tt>null</tt> if the translation cannot
     * be performed
     */
     ...
     *
     * @return true if the context initialization parameter with the given name
     * and value was set successfully on this ServletContext, and false if it
     * was not set because this ServletContext already contains a context
     ...
     *
     * <p>
     * Use the {@link #getAttribute} method with an attribute name to get the
    ...
    /**
     * Binds an object to a given attribute name in this ServletContext. If the
    ...
    /**
     * Removes the attribute with the given name from this ServletContext. After
     ...
     *
     * <p>
     * The specified <tt>className</tt> will be loaded using the classloader
     * associated with the application represented by this ServletContext.
     *
     * <p>
     * If this ServletContext already contains a preliminary ServletRegistration
     * for a servlet with the given
     * <tt>servletName</tt>, it will be completed (by assigning the given
     * <tt>className</tt> to it) and returned.
     *
     * <p>
     ...
     * <tt>javax.annotation.security.DeclareRoles</tt> annotations. In addition,
     * this method supports resource injection if the class with the given
     * <tt>className</tt> represents a Managed Bean. See the Java EE platform
     * and JSR 299 specifications for additional details about Managed Beans and
     * resource injection.
     *
     ...
     * configure the registered servlet, or
     * <tt>null</tt> if this ServletContext already contains a complete
     * ServletRegistration for a servlet with the given <tt>servletName</tt>
     *
     * @throws IllegalStateException if this ServletContext has already been
     * initialized
     *
     * @throws IllegalArgumentException if <code>servletName</code> is null or
     * an empty String
     *
     * @throws UnsupportedOperationException if this ServletContext was passed
     * to the {@link ServletContextListener#contextInitialized} method of a
     ...
     * If this ServletContext already contains a preliminary ServletRegistration
     * for a servlet with the given
     * <tt>servletName</tt>, it will be completed (by assigning the class name
     * of the given servlet instance to it) and returned.
     *
     * @param servletName the name of the servlet
     * @param servlet the servlet instance to register
     *
     * @return a ServletRegistration object that may be used to further
     * configure the given servlet, or
     * <tt>null</tt> if this ServletContext already contains a complete
     * ServletRegistration for a servlet with the given <tt>servletName</tt> or
     * if the same servlet instance has already been registered with this or
     * another ServletContext in the same container
     *
     * @throws IllegalStateException if this ServletContext has already been
     * initialized
     *
     ...
     * {@link SingleThreadModel}, or <code>servletName</code> is null or an
     * empty String
     *
     * @since Servlet 3.0
     */
     ...
     * If this ServletContext already contains a preliminary ServletRegistration
     * for a servlet with the given
     * <tt>servletName</tt>, it will be completed (by assigning the name of the
     * given
     ...
     * <tt>servletClass</tt> represents a Managed Bean. See the Java EE platform
     * and JSR 299 specifications for additional details about Managed Beans and
     * resource injection.
     *
     * @param servletName the name of the servlet
     ...
     * configure the registered servlet, or
     * <tt>null</tt> if this ServletContext already contains a complete
     * ServletRegistration for the given
     * <tt>servletName</tt>
     *
     * @throws IllegalStateException if this ServletContext has already been
     * initialized
     ...
     *
     * <p>
     * This method introspects the given <tt>clazz</tt> for the following
     * annotations: null null null null null null null null null null null null
     * null null null null     {@link javax.servlet.annotation.ServletSecurity}, 
     * {@link javax.servlet.annotation.MultipartConfig},
     * <tt>javax.annotation.security.RunAs</tt>, and
     * <tt>javax.annotation.security.DeclareRoles</tt>. In addition, this method
     * supports resource injection if the given <tt>clazz</tt> represents a
     * Managed Bean. See the Java EE platform and JSR 299 specifications for
     ...
     * <tt>servletName</tt>.
     *
     * @return the (complete or preliminary) ServletRegistration for the servlet
     * with the given
     * <tt>servletName</tt>, or null if no ServletRegistration exists under that
     * name
     *
     * @throws UnsupportedOperationException if this ServletContext was passed
     * to the {@link ServletContextListener#contextInitialized} method of a
     ...
     *
     * <p>
     * The returned Map includes the ServletRegistration objects corresponding
     * to all declared and annotated servlets, as well as the
     * ServletRegistration objects corresponding to all servlets that have been
     * added via one of the <tt>addServlet</tt> methods.
     *
     * <p>
     * If permitted, any changes to the returned Map must not affect this
     * ServletContext.
     *
     * @return Map of the (complete and preliminary) ServletRegistration objects
     * corresponding to all servlets currently registered with this
     * ServletContext
     *
     * @throws UnsupportedOperationException if this ServletContext was passed
     * to the {@link ServletContextListener#contextInitialized} method of a
     ...
     * for a filter with the given
     * <tt>filterName</tt>, it will be completed (by assigning the given
     * <tt>className</tt> to it) and returned.
     *
     * <p>
     * This method supports resource injection if the class with the given
     ...
     *
     * @return a FilterRegistration object that may be used to further configure
     * the registered filter, or
     * <tt>null</tt> if this ServletContext already contains a complete
     * FilterRegistration for a filter with the given
     * <tt>filterName</tt>
     *
     * @throws IllegalStateException if this ServletContext has already been
     * initialized
     *
     * @throws IllegalArgumentException if <code>filterName</code> is null or an
     * empty String
     *
     * @throws UnsupportedOperationException if this ServletContext was passed
     * to the {@link ServletContextListener#contextInitialized} method of a
     ...
     * If this ServletContext already contains a preliminary FilterRegistration
     * for a filter with the given
     * <tt>filterName</tt>, it will be completed (by assigning the class name of
     * the given filter instance to it) and returned.
     *
     * @param filterName the name of the filter
     ...
     * <tt>null</tt> if this ServletContext already contains a complete
     * FilterRegistration for a filter with the given
     * <tt>filterName</tt> or if the same filter instance has already been
     * registered with this or another ServletContext in the same container
     *
     * @throws IllegalStateException if this ServletContext has already been
     * initialized
     *
     ...
     * If this ServletContext already contains a preliminary FilterRegistration
     * for a filter with the given
     * <tt>filterName</tt>, it will be completed (by assigning the name of the
     * given
     * <tt>filterClass</tt> to it) and returned.
     *
     * <p>
     * This method supports resource injection if the given
     * <tt>filterClass</tt> represents a Managed Bean. See the Java EE platform
     * and JSR 299 specifications for additional details about Managed Beans and
     * resource injection.
     *
     * @param filterName the name of the filter
     ...
     *
     * <p>
     * The returned Filter instance may be further customized before it is
     * registered with this ServletContext via a call to
     * {@link #addFilter(String,Filter)}.
     *
     * <p>
     * The given Filter class must define a zero argument constructor, which is
     * used to instantiate it.
     *
     * <p>
     * This method supports resource injection if the given
     * <tt>clazz</tt> represents a Managed Bean. See the Java EE platform and
     * JSR 299 specifications for additional details about Managed Beans and
     * resource injection.
     *
     ...
     * <tt>filterName</tt>.
     *
     * @return the (complete or preliminary) FilterRegistration for the filter
     * with the given
     * <tt>filterName</tt>, or null if no FilterRegistration exists under that
     * name
     *
     * @throws UnsupportedOperationException if this ServletContext was passed
     * to the {@link ServletContextListener#contextInitialized} method of a
     ...
     *
     * <p>
     * The returned Map includes the FilterRegistration objects corresponding to
     * all declared and annotated filters, as well as the FilterRegistration
     * objects corresponding to all filters that have been added via one of the
     * <tt>addFilter</tt> methods.
     *
     * <p>
     * Any changes to the returned Map must not affect this ServletContext.
     *
     * @return Map of the (complete and preliminary) FilterRegistration objects
     * corresponding to all filters currently registered with this
     * ServletContext
     *
     * @throws UnsupportedOperationException if this ServletContext was passed
     * to the {@link ServletContextListener#contextInitialized} method of a
     ...
     *
     * @return the <tt>SessionCookieConfig</tt> object through which various
     * properties of the session tracking cookies created on behalf of this
     * <tt>ServletContext</tt> may be configured
     *
     * @throws UnsupportedOperationException if this ServletContext was passed
     ...
     *
     * <p>
     * The given <tt>sessionTrackingModes</tt> replaces any session tracking
     * modes set by a previous invocation of this method on this
     * <tt>ServletContext</tt>.
     *
     * @param sessionTrackingModes the set of session tracking modes to become
     * effective for this
     * <tt>ServletContext</tt>
     *
     * @throws IllegalStateException if this ServletContext has already been
     ...
     * <tt>SessionTrackingMode.SSL</tt>, or if
     * <tt>sessionTrackingModes</tt> specifies a session tracking mode that is
     * not supported by the servlet container
     *
     * @since Servlet 3.0
     */
     ...
     *
     * <p>
     * The class with the given name will be loaded using the classloader
     * associated with the application represented by this ServletContext, and
     * must implement one or more of the following interfaces:
     * <ul>
     ...
     * {@link ServletContainerInitializer#onStartup}, then the class with the
     * given name may also implement {@link ServletContextListener}, in addition
     * to the interfaces listed above.
     *
     * <p>
     * As part of this method call, the container must load the class with the
     * specified class name to ensure that it implements one of the required
     * interfaces.
     *
     * <p>
     ...
     * implements {@link ServletRequestListener},
     * {@link ServletContextListener}, or
     * {@link javax.servlet.http.HttpSessionListener}), then the new listener
     * will be added to the end of the ordered list of listeners of that
     * interface.
     *
     * <p>
     * This method supports resource injection if the class with the given
     ...
     *
     * @throws IllegalArgumentException if the class with the given name does
     * not implement any of the above interfaces, or if it implements
     * {@link ServletContextListener} and this ServletContext was not passed to
     ...
     *
     * <p>
     * The given listener must be an instance of one or more of the following
     * interfaces:
     * <ul>
     * <li>{@link ServletContextAttributeListener}</tt>
     * <li>{@link ServletRequestListener}</tt>
     ...
     * <p>
     * If this ServletContext was passed to
     * {@link ServletContainerInitializer#onStartup}, then the given listener
     * may also be an instance of {@link ServletContextListener}, in addition to
     ...
     * instance of {@link ServletRequestListener},
     * {@link ServletContextListener}, or
     * {@link javax.servlet.http.HttpSessionListener}), then the listener will
     * be added to the end of the ordered list of listeners of that interface.
     *
     * @param t the listener to be added
     *
     * @throws IllegalArgumentException if the given listener is not an instance
     * of any of the above interfaces, or if it is an instance of
     * {@link ServletContextListener} and this ServletContext was not passed to
     * {@link ServletContainerInitializer#onStartup}
     *
     * @throws IllegalStateException if this ServletContext has already been
     ...
     * <tt>listenerClass</tt> may also implement {@link ServletContextListener},
     * in addition to the interfaces listed above.
     *
     * <p>
     * If the given <tt>listenerClass</tt> implements a listener interface whose
     * invocation order corresponds to the declaration order (i.e., if it
     * implements {@link ServletRequestListener},
     * {@link ServletContextListener}, or
     * {@link javax.servlet.http.HttpSessionListener}), then the new listener
     ...
     * <p>
     * This method supports resource injection if the given
     * <tt>listenerClass</tt> represents a Managed Bean. See the Java EE
     * platform and JSR 299 specifications for additional details about Managed
     * Beans and resource injection.
     *
     ...
     *
     * <p>
     * The returned EventListener instance may be further customized before it
     * is registered with this ServletContext via a call to
     * {@link #addListener(EventListener)}.
     *
     * <p>
     * The given EventListener class must define a zero argument constructor,
    ...
    /**
     * Gets the <code>&lt;jsp-config&gt;</code> related configuration that was
     * aggregated from the <code>web.xml</code> and
     * <code>web-fragment.xml</code> descriptor files of the web application
     * represented by this ServletContext.
     *
     * @return the <code>&lt;jsp-config&gt;</code> related configuration that
     * was aggregated from the <code>web.xml</code> and
     * <code>web-fragment.xml</code> descriptor files of the web application
     ...
     *
     * <p>
     * If a security manager exists, and the caller's class loader is not the
     * same as, or an ancestor of the requested class loader, then the security
     * manager's <code>checkPermission</code> method is called with a
     * <code>RuntimePermission("getClassLoader")</code> permission to check
     * whether access to the requested class loader should be granted.
     *
     ...
     * {@link javax.servlet.annotation.WebListener}
     *
     * @throws SecurityException if a security manager denies access to the
     * requested class loader
     *
     * @since Servlet 3.0
     */
     ...
     *
     * <p>
     * Roles that are implicitly declared as a result of their use within the {@link ServletRegistration.Dynamic#setServletSecurity
     ...
     * {@link javax.servlet.annotation.WebListener}
     *
     * @throws IllegalArgumentException if any of the argument roleNames is null
     * or the empty string
     *
     ...
     * information with the logical host. The returned value is NOT expected or
     * required to be equivalent to a network address or hostname of the logical
     * host.
     *
