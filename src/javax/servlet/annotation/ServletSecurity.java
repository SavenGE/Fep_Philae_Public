...
public @interface ServletSecurity {
    ...
    /**
     * Defines the data protection requirements that must be satisfied by the
     * transport
     */
    enum TransportGuarantee {
        ...
        NONE,
        /**
         * All user data must be encrypted by the transport (typically using
         * SSL/TLS).
         */
        CONFIDENTIAL
    }
    ...
    /**
     * Get the {@link HttpConstraint} that defines the protection that is to be
     * applied to all HTTP methods that are NOT represented in the array
     * returned by <tt>httpMethodConstraints</tt>.
     *
     ...
     * {@link HttpMethodConstraint} names an HTTP protocol method and defines
     * the protection to be applied to it.
     *
     * @return an array of {@link HttpMethodConstraint} elements each defining
     * the protection to be applied to one HTTP protocol method. For any HTTP
     * method name, there must be at most one corresponding element in the
     * returned array. If the returned array is of zero length, it indicates
