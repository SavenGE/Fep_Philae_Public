...
/**
 * <p>
 * An abstract implementation of a timeout.</p>
 * <p>
 * Subclasses should implement {@link #onTimeoutExpired()}.</p>
 * <p>
 * This implementation is optimised assuming that the timeout will mostly be
 * cancelled and then reused with a similar value.</p>
 */
public abstract class CyclicTimeout implements Destroyable {
    ...
    public boolean schedule(long delay, TimeUnit units) {
        ...
        while (true) {
            ...
            if (wakeup == null || wakeup._at > new_timeout_at) // No, we need an earlier wakeup.
            {
    ...
    /**
     * <p>
     * Cancels this CyclicTimeout so that it won't expire.</p>
     * <p>
     * After being cancelled, this CyclicTimeout can be scheduled again.</p>
     *
    ...
    /**
     * <p>
     * Invoked when the timeout expires.</p>
     */
    public abstract void onTimeoutExpired();

    /**
     * <p>
     * Destroys this CyclicTimeout.</p>
     * <p>
     * After being destroyed, this CyclicTimeout is not used anymore.</p>
     */
    @Override
    public void destroy() {
    ...
    private class Wakeup implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (wakeup == null) // Not found, we become a noop.
                {
                    return;
                }
                ...
                if (timeout._at <= now) {
                    ...
                    // Is the current wakeup good to use? ie before our timeout time?
                    if (wakeup == null || wakeup._at >= timeout._at) // No, we need an earlier wakeup.
                    {
