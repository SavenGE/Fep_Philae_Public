...
abstract public class WriteFlusher {
    ...
    private static final ByteBuffer[] EMPTY_BUFFERS = new ByteBuffer[]{
     ...
     * @throws WritePendingException if currentState is WRITING and new state is
     * WRITING (api usage error)
     */
    ...
    /**
     * In CompletingState WriteFlusher is flushing buffers that have not been
     * fully written in write(). If write() didn't flush all buffers in one go,
     * it'll switch the State to PendingState. completeWrite() will then switch
     * to this state and try to flush the remaining buffers.
     */
    ...
    /**
     * In PendingState not all buffers could be written in one go. Then write()
     * will switch to PendingState() and preserve the state by creating a new
    ...
    /**
     * Abstract call to be implemented by specific WriteFlushers. It should
     * schedule a call to {@link #completeWrite()} or {@link #onFail(Throwable)}
     * when appropriate.
     */
    abstract protected void onIncompleteFlush();

    /**
     * Tries to switch state to WRITING. If successful it writes the given
     * buffers to the EndPoint. If state transition fails it'll fail the
     * callback.
     *
     ...
     * @throws WritePendingException if unable to write due to prior pending
     * write
     */
     ...
     * {@link #onIncompleteFlush()} to request a call to this method when a call
     * to {@link EndPoint#flush(ByteBuffer...)} is likely to be able to
     * progress.
     *
     * It tries to switch from PENDING to COMPLETING. If state transition fails,
     * then it does nothing as the callback should have been already failed.
     * That's because the only way to switch from PENDING outside this method is
     ...
     * @return true if the flusher passed the failure to a {@link Callback}
     * instance
     */
    ...
    /**
     * <p>
     * A listener of {@link WriteFlusher} events.</p>
     */
    public interface Listener {

        /**
         * <p>
         * Invoked when a {@link WriteFlusher} flushed bytes in a non-blocking
         * way, as part of a - possibly larger - write.</p>
         * <p>
         * This method may be invoked multiple times, for example when writing a
         * large buffer: a first flush of bytes, then the connection became TCP
         * congested, and a subsequent flush of bytes when the connection became
         * writable again.</p>
         * <p>
         * This method is never invoked concurrently, but may be invoked by
         * different threads, so implementations may not rely on thread-local
         * variables.</p>
         * <p>
         * Implementations may throw an {@link IOException} to signal that the
         * write should fail, for example if the implementation enforces a
         * minimum data rate.</p>
         *
