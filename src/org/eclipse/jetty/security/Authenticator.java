 ...
 * {@link Authentication} are returned in order to signal the next step in
 * authentication.
 *
...
public interface Authenticator {
    ...
    /* ------------------------------------------------------------ */
    /**
     * Called prior to validateRequest. The authenticator can manipulate the
     * request to update it with information that can be inspected prior to
     * validateRequest being called. The primary purpose of this method is to
     * satisfy the Servlet Spec 3.1 section 13.6.3 on handling Form
     * authentication where the http method of the original request causing
     * authentication is not the same as the http method resulting from the
     ...
     * @return An Authentication. If Authentication is successful, this will be
     * a {@link org.eclipse.jetty.server.Authentication.User}. If a response has
     * been sent by the Authenticator (which can be done for both successful and
     * unsuccessful authentications), then the result will implement
     * {@link org.eclipse.jetty.server.Authentication.ResponseSent}. If
     * Authentication is not manditory, then a
