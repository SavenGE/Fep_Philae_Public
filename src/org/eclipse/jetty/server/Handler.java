 ...
 * <p>
 * A Handler instance is required by a {@link Server} to handle incoming HTTP
 * requests.
 * <p>
 * A Handler may:
 * <ul>
 * <li>Completely generate the HTTP Response</li>
 * <li>Examine/modify the request and call another Handler (see
 * {@link HandlerWrapper}).
 * <li>Pass the request to one or more other Handlers (see
 * {@link HandlerCollection}).
 * </ul>
 *
 * Handlers are passed the servlet API request and response object, but are not
 * Servlets. The servlet container is implemented by handlers for context,
 * security, session and servlet that modify the request object before passing
 * it to the next stage of handling.
 *
 */
