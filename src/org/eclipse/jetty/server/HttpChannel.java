...
/**
 * HttpChannel represents a single endpoint for HTTP semantic processing. The
 * HttpChannel is both a HttpParser.RequestHandler, where it passively receives
 * events from an incoming HTTP request, and a Runnable, where it actively takes
 * control of the request/response life cycle and calls the application (perhaps
 * suspending and resuming with multiple calls to run). The HttpChannel signals
 * the switch from passive mode to active mode by returning true to one of the
 * HttpParser.RequestHandler callbacks. The completion of the active phase is
 * signalled by a call to HttpTransport.completed().
 */
public class HttpChannel implements Runnable, HttpOutput.Interceptor {
     ...
     * <p>
     * This is implemented as a call to {@link EndPoint#getIdleTimeout()}, but
     * may be overridden by channels that have timeouts different from their
     * connections.
     *
     ...
     * <p>
     * This is implemented as a call to {@link EndPoint#setIdleTimeout(long)},
     * but may be overridden by channels that have timeouts different from their
     * connections.
     *
     ...
     * then accessing the input stream indicates that the handler/servlet is
     * ready for the request body and thus a 100 Continue response is sent.
     *
    ...
    /**
     * @return True if the channel is ready to continue handling (ie it is not
     * suspended)
     */
    ...
    /**
     * <p>
     * Sends an error 500, performing a special logic to detect whether the
     * request is suspended, to avoid concurrent writes from the
     * application.</p>
     * <p>
     * It may happen that the application suspends, and then throws an
     * exception, while an application spawned thread writes the response
     * content; in such case, we attempt to commit the error directly bypassing
     * the {@link ErrorHandler} mechanisms and the response OutputStream.</p>
     *
    ...
    /**
     * <p>
     * Non-Blocking write, committing the response if needed.</p>
    ...
    /**
     * @return true if the HttpChannel can efficiently use direct buffer
     * (typically this means it is not over SSL or a multiplexed protocol)
     */
    ...
    /**
     * If a write or similar operation to this channel fails, then this method
     * should be called.
     * <p>
    ...
    /**
     * <p>
     * Listener for {@link HttpChannel} events.</p>
     * <p>
     ...
     * <p>
     * The events parameters, especially the {@link Request} object, may be in a
     * transient state depending on the event, and not all properties/features
     * of the parameters may be available inside a listener method.</p>
     * <p>
     * It is recommended that the event parameters are <em>not</em> acted upon
     * in the listener methods, or undefined behavior may result. For example,
     * it would be a bad idea to try to read some content from the
     * {@link javax.servlet.ServletInputStream} in listener methods. On the
     * other hand, it is legit to store request attributes in one listener
     * method that may be possibly retrieved in another listener method in a
     * later event.</p>
     * <p>
    ...
    public interface Listener {
        ...
        /**
         * Invoked just after the HTTP request line and headers have been
         * parsed.
         *
        ...
        /**
         * Invoked every time a request content chunk has been parsed, just
         * before making it available to the application.
         *
         * @param request the request object
         * @param content a {@link ByteBuffer#slice() slice} of the request
         * content chunk
         */
        ...
        /**
         * Invoked after a response content chunk has been written to the
         * network.
         *
         * @param request the request object
         * @param content a {@link ByteBuffer#slice() slice} of the response
         * content chunk
         */
        ...
        /**
         * Invoked when the request <em>and</em> response processing are
         * complete.
         *
         * @param request the request object
         */
