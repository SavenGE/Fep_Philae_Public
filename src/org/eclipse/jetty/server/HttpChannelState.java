...
/**
 * Implementation of AsyncContext interface that holds the state of
...
public class HttpChannelState {
    ...
    /**
     * Signal that the HttpConnection has finished handling the request. For
     * blocking connectors, this call may block if the request has been
     * suspended (startAsync called).
     *
     * @return next actions be handled again (eg because of a resume that
     * happened before unhandle was called)
     */
    ...
    /**
     * Called to signal async read isReady() has returned false. This indicates
     * that there is no content available to be consumed and that once the
     * channel enters the ASYNC_WAIT state it will register for read interest by
     * calling {@link HttpChannel#onAsyncWaitForContent()} either from this
     * method or from a subsequent call to {@link #unhandle()}.
     */
    ...
    /**
     * Called to signal that content is now available to read. If the channel is
     * in ASYNC_WAIT state and unready (ie isReady() has returned false), then
     * the state is changed to ASYNC_WOKEN and true is returned.
     *
    ...
    /**
     * Called to signal that the channel is ready for a callback. This is
     * similar to calling {@link #onReadUnready()} followed by
     * {@link #onContentAdded()}, except that as content is already available,
     * read interest is never set.
     *
    ...
    /**
     * Called to indicate that more content may be available, but that a
     * handling thread may need to produce (fill/parse) it. Typically called by
     * the async read success callback.
     *
    ...
    /**
     * Called to signal that a read has read -1. Will wake if the read was
     * called while in ASYNC_WAIT state
     *
