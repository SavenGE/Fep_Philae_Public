 ...
 * {@link HttpChannel} class. Typically a HTTPConfiguration instance is
 * instantiated and passed to a {@link ConnectionFactory} that can create HTTP
 * channels (e.g. HTTP, AJP or FCGI).</p>
 * <p>
 * The configuration held by this class is not for the wire protocol, but for
 * the interpretation and handling of HTTP requests that could be transported by
...
public class HttpConfiguration {
    ...
    /**
     * <p>
     * An interface that allows a request object to be customized for a
     * particular HTTP connector configuration. Unlike Filters, customizer are
     * applied before the request is submitted for processing and can be
     * specific to the connector on which the request was received.</p>
     *
     * <p>
     * Typically Customizers perform tasks such as:</p>
     * <ul>
     * <li>process header fields that may be injected by a proxy or load
     * balancer.
     * <li>setup attributes that may come from the connection/connector such as
     * SSL Session IDs
     * <li>Allow a request to be marked as secure or authenticated if those have
     * been offloaded and communicated by header, cookie or other out-of-band
     * mechanism
     * <li>Set request attributes/fields that are determined by the connector on
    ...
    /**
     * <p>
     * Adds a {@link Customizer} that is invoked for every request received.</p>
     * <p>
     * Customizers are often used to interpret optional headers (eg
     * {@link ForwardedRequestCustomizer}) or optional protocol semantics (eg
     * {@link SecureRequestCustomizer}).
     *
     ...
     * delayed dispatch.</p>
     *
     * @return the max idle time in ms or if == 0 implies an infinite timeout,
     * &lt;0 implies no HTTP channel timeout and the connection timeout is used
     * instead.
     */
     ...
     * delayed dispatch.</p>
     *
     * @param timeoutMs the max idle time in ms or if == 0 implies an infinite
     * timeout, &lt;0 implies no HTTP channel timeout and the connection timeout
     * is used instead.
     */
    ...
    /**
     * <p>
     * This timeout is in addition to the {@link Connector#getIdleTimeout()},
     * and applies to the total operation (as opposed to the idle timeout that
     * applies to the time no data is being sent). This applies only to blocking
     * operations and does not affect asynchronous read and write.</p>
     *
     * @return -1, for no blocking timeout (default), 0 for a blocking timeout
     * equal to the idle timeout; &gt;0 for a timeout in ms applied to the total
     * blocking operation.
     */
     ...
     * operations and does not affect asynchronous read and write.</p>
     *
     * @param blockingTimeout -1, for no blocking timeout (default), 0 for a
     * blocking timeout equal to the idle timeout; &gt;0 for a timeout in ms
     * applied to the total blocking operation.
     */
    ...
    /**
     * @param delay if true, delay the application dispatch until content is
    ...
    /**
     * <p>
     * Sets the {@link Customizer}s that are invoked for every request
     * received.</p>
     * <p>
     * Customizers are often used to interpret optional headers (eg
     * {@link ForwardedRequestCustomizer}) or optional protocol semantics (eg
     * {@link SecureRequestCustomizer}).</p>
     *
     ...
     * before being sent to the client. A larger buffer can improve performance
     ...
     * can make a server more vulnerable to denial of service attacks.</p>
     *
     ...
     * headers (eg for redirection). However, larger headers will also consume
     * more memory.</p>
     *
     * @param responseHeaderSize the maximum size in bytes of the response
     * header
     */
    ...
    /**
     * <p>
     * Sets the TCP/IP port used for CONFIDENTIAL and INTEGRAL redirections.</p>
     *
    ...
    /**
     * <p>
     * Set the URI scheme used for CONFIDENTIAL and INTEGRAL redirections.</p>
     *
    ...
    /**
     * Tests whether the HTTP method supports {@code x-www-form-urlencoded}
     * content
     *
     * @param method the HTTP method
     * @return true if requests with this method can be decoded as
     * {@code x-www-form-urlencoded} content to be made available via the
     * {@link Request#getParameter(String)} and associated APIs
     */
    ...
    /**
     * @return The maximum error dispatches for a request to prevent looping on
     * an error
     */
    ...
    /**
     * @param max The maximum error dispatches for a request to prevent looping
     * on an error
     */
    ...
    /**
     * @return The minimum request data rate in bytes per second; or &lt;=0 for
     * no limit
     */
    ...
    /**
     * @param bytesPerSecond The minimum request data rate in bytes per second;
     * or &lt;=0 for no limit
     */
    ...
    /**
     * @return The minimum response data rate in bytes per second; or &lt;=0 for
     * no limit
     */
    ...
    /**
     * <p>
     * Sets an minimum response content data rate.</p>
     * <p>
     * The value is enforced only approximately - not precisely - due to the
     * fact that for efficiency reasons buffer writes may be comprised of both
     * response headers and response content.</p>
     *
     * @param bytesPerSecond The minimum response data rate in bytes per second;
     * or &lt;=0 for no limit
     */
    ...
    /**
     * @param notifyRemoteAsyncErrors whether remote errors, when detected, are
     * notified to async applications
     */
    ...
    /**
     * @return whether remote errors, when detected, are notified to async
     * applications
     */
