...
/**
 * <p>
 * A {@link Connection} that handles the HTTP protocol.</p>
 */
public class HttpConnection extends AbstractConnection implements Runnable, HttpTransport, Connection.UpgradeFrom, WriteFlusher.Listener {
    ...
    /**
     * Get the current connection that this thread is dispatched to. Note that a
     * thread may be processing a request asynchronously and thus not be
     * dispatched to the connection.
     *
     * @return the current HttpConnection or null
     * @see Request#getAttribute(String) for a more general way to access the
     * HttpConnection
     */
    ...
    public void send(MetaData.Response info, boolean head, ByteBuffer content, boolean lastContent, Callback callback) {
        ...
        } else // If we are still expecting a 100 continues when we commit
        if (_channel.isExpecting100Continue()) // then we can't be persistent
        {
