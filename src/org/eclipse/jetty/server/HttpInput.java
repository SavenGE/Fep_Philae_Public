...
/**
 * {@link HttpInput} provides an implementation of {@link ServletInputStream}
 * for {@link HttpChannel}.
 * <p>
 * Content may arrive in patterns such as [content(), content(),
 * messageComplete()] so that this class maintains two states: the content state
 * that tells whether there is content to consume and the EOF state that tells
 * whether an EOF has arrived. Only once the content has been consumed the
 * content state is moved to the EOF state.
 * </p>
 */
public class HttpInput extends ServletInputStream implements Runnable {
     ...
     * <p>
     * Unlike InputStream wrappers that can be applied by filters, an
     * interceptor is entirely transparent and works with async IO APIs.
     * </p>
     * <p>
     * An Interceptor may consume data from the passed content and the
     * interceptor will continue to be called for the same content until the
     * interceptor returns null or an empty content. Thus even if the passed
     * content is completely consumed the interceptor will be called with the
     * same content until it can no longer produce more content.
     * </p>
     *
    ...
    public interface Interceptor {
        ...
        /**
         * @param content The content to be intercepted (may be empty or a
         * {@link SentinelContent}. The content will be modified with any data
         * the interceptor consumes, but there is no requirement that all the
         * data is consumed by the interceptor.
         * @return The intercepted content or null if interception is completed
         * for that content.
         */
    ...
    /**
     * An {@link Interceptor} that chains two other {@link Interceptor}s
     * together. The {@link Interceptor#readFrom(Content)} calls the previous
     * {@link Interceptor}'s {@link Interceptor#readFrom(Content)} and then
     * passes any {@link Content} returned to the next {@link Interceptor}.
     */
    ...
    /**
     * Set the {@link Interceptor}, using a {@link ChainedInterceptor} if an
     * {@link Interceptor} is already set.
     *
    ...
    /**
     * Called when derived implementations should attempt to produce more
     * Content and add it via {@link #addContent(Content)}. For protocols that
     * are constantly producing (eg HTTP2) this can be left as a noop;
     *
    ...
    /**
     * Poll the inputQ for Content. Consumed buffers and
     * {@link SentinelContent}s are removed and EOF state updated if need be.
     *
    ...
    /**
     * Poll the inputQ for Content or EOF. Consumed buffers and non EOF
     * {@link SentinelContent}s are removed. EOF state is not updated.
    ...
    /**
     * Consumes the given content. Calls the content succeeded if all content
     * consumed.
     *
    ...
    /**
     * This method should be called to signal that an EOF has been detected
     * before all the expected content arrived.
     * <p>
     * Typically this will result in an EOFException being thrown from a
     * subsequent read rather than a -1 return.
     *
     * @return true if content channel woken for read
     */
    ...
    /**
     * This method should be called to signal that all the expected content
     * arrived.
     *
     * @return true if content channel woken for read
     */
    ...
    /**
     * A Sentinel Content, which has zero length content but indicates some
     * other event in the input stream (eg EOF)
     *
     */
