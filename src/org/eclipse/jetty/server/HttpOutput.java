...
/**
 * <p>
 * {@link HttpOutput} implements {@link ServletOutputStream} as required by the
 * Servlet specification.</p>
 * <p>
 * {@link HttpOutput} buffers content written by the application until a further
 * write will overflow the buffer, at which point it triggers a commit of the
 * response.</p>
 * <p>
 * {@link HttpOutput} can be closed and reopened, to allow requests included via
 * {@link RequestDispatcher#include(ServletRequest, ServletResponse)} to close
 * the stream, to be reopened after the inclusion ends.</p>
 */
public class HttpOutput extends ServletOutputStream implements Runnable {

    /**
     * The HttpOutput.Interceptor is a single intercept point for all output
     * written to the HttpOutput: via writer; via output stream; asynchronously;
     * or blocking.
     * <p>
     * The Interceptor can be used to implement translations (eg Gzip) or
     * additional buffering that acts on all output. Interceptors are created in
     * a chain, so that multiple concerns may intercept.
     * <p>
     * The {@link HttpChannel} is an {@link Interceptor} and is always the last
     * link in any Interceptor chain.
     * <p>
     * Responses are committed by the first call to
     * {@link #write(ByteBuffer, boolean, Callback)} and closed by a call to
     * {@link #write(ByteBuffer, boolean, Callback)} with the last boolean set
     * true. If no content is available to commit or close, then a null buffer
     * is passed.
     */
    public interface Interceptor {

        /**
         * Write content. The response is committed by the first call to write
         * and is closed by a call with last == true. Empty content buffers may
         * be passed to force a commit or close.
         *
         * @param content The content to be written or an empty buffer.
         * @param last True if this is the last call to write
         * @param callback The callback to use to indicate
         * {@link Callback#succeeded()} or {@link Callback#failed(Throwable)}.
         */
        void write(ByteBuffer content, boolean last, Callback callback);

        /**
         * @return The next Interceptor in the chain or null if this is the last
         ...
         * {@link ByteBuffer}s in the
         * {@link #write(ByteBuffer, boolean, Callback)} method. If false is
         * returned, then passing direct buffers may cause inefficiencies.
         */
    ...
    /**
     * Called to indicate that the last write has been performed. It updates the
     * state and performs cleanup operations.
     */
    void closed() {
        while (true) {
            OutputState state = _state.get();
    ...
    /**
     * Asynchronous send of stream content. The stream will be closed after
     * reading all content.
     *
     * @param in The stream content to send
     * @param callback The callback to use to notify success or failure
    ...
    /**
     * Asynchronous send of channel content. The channel will be closed after
     * reading all content.
     *
     * @param in The channel content to send
     * @param callback The callback to use to notify success or failure
    ...
    /**
     * <p>
     * Invoked when bytes have been flushed to the network.</p>
     * <p>
     * The number of flushed bytes may be different from the bytes written by
     * the application if an {@link Interceptor} changed them, for example by
     * compressing them.</p>
     *
    ...
    /**
     * An iterating callback that will take content from an InputStream and
     * write it to the associated {@link HttpChannel}. A non direct buffer of
     * size {@link HttpOutput#getBufferSize()} is used. This callback is passed
     * to the {@link HttpChannel#write(ByteBuffer, boolean, Callback)} to be
     * notified as each buffer is written and only once all the input is
     * consumed will the wrapped {@link Callback#succeeded()} method be called.
     */
    ...
    /**
     * An iterating callback that will take content from a ReadableByteChannel
     * and write it to the {@link HttpChannel}. A {@link ByteBuffer} of size
     * {@link HttpOutput#getBufferSize()} is used that will be direct if
     * {@link HttpChannel#useDirectBuffers()} is true. This callback is passed
     * to the {@link HttpChannel#write(ByteBuffer, boolean, Callback)} to be
     * notified as each buffer is written and only once all the input is
     * consumed will the wrapped {@link Callback#succeeded()} method be called.
     */
