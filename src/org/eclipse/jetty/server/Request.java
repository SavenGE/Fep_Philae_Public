 ...
 * Jetty Request.
 * <p>
 * Implements {@link javax.servlet.http.HttpServletRequest} from the
 * <code>javax.servlet.http</code> package.
 * </p>
 * <p>
 * The standard interface of mostly getters, is extended with setters so that
 * the request is mutable by the handlers that it is passed to. This allows the
 * request object to be as lightweight as possible and not actually implement
 * any significant behavior. For example
 * <ul>
 *
 * <li>The {@link Request#getContextPath()} method will return null, until the
 * request has been passed to a {@link ContextHandler} which matches the
 * {@link Request#getPathInfo()} with a context path and calls
 * {@link Request#setContextPath(String)} as a result.</li>
 *
 * <li>the HTTP session methods will all return null sessions until such time as
 * a request has been passed to a
 * {@link org.eclipse.jetty.server.session.SessionHandler} which checks for
 * session cookies and enables the ability to create new sessions.</li>
 *
 * <li>The {@link Request#getServletPath()} method will return null until the
 * request has been passed to a
 * <code>org.eclipse.jetty.servlet.ServletHandler</code> and the pathInfo
 * matched against the servlet URL patterns and
 * {@link Request#setServletPath(String)} called as a result.</li>
 * </ul>
 *
 * <p>
 * A request instance is created for each connection accepted by the server and
 * recycled for each HTTP request received via that connection. An effort is
 * made to avoid reparsing headers and cookies that are likely to be the same
 * for requests from the same connection.
 * </p>
 * <p>
 * Request instances are recycled, which combined with badly written
 * asynchronous applications can result in calls on requests that have been
 * reset. The code is written in a style to avoid NPE and ISE when such calls
 * are made, as this has often proved generate exceptions that distraction from
 * debugging such bad asynchronous applications. Instead, request methods
 * attempt to not fail when called in an illegal state, so that hopefully the
 * bad application will proceed to a major state event (eg calling
 * AsyncContext.onComplete) which has better asynchronous guards, true atomic
 * state and better failure behaviour that will assist in debugging.
 * </p>
 * <p>
 * The form content that a request can process is limited to protect from Denial
 * of Service attacks. The size in bytes is limited by
 * {@link ContextHandler#getMaxFormContentSize()} or if there is no context then
 * the "org.eclipse.jetty.server.Request.maxFormContentSize" {@link Server}
 * attribute. The number of parameters keys is limited by
 * {@link ContextHandler#getMaxFormKeys()} or if there is no context then the
 * "org.eclipse.jetty.server.Request.maxFormKeys" {@link Server} attribute.
 * </p>
 * <p>
 * If IOExceptions or timeouts occur while reading form parameters, these are
 * thrown as unchecked Exceptions: ether {@link RuntimeIOException},
...
public class Request implements HttpServletRequest {
     ...
     * @return True if the inputParameters reference is equal to NO_PARAMS
     * otherwise False
     */
     ...
     * <li>Referrer headers
     * </ul></li>
     * <li>If the request was Authenticated, an Authorization header will be set
     * with a container generated token that will result in equivalent
     * Authorization</li>
     * <li>The query string from {@link #getQueryString()}
     * <li>The {@link #getRequestedSessionId()} value, unless at the time of the
     * call {@link #getSession(boolean)} has previously been called to create a
     * new {@link HttpSession}, in which case the new session ID will be used as
     * the PushBuilders requested session ID.</li>
     * <li>The source of the requested session id will be the same as for this
     * request</li>
     * <li>The builders Referer header will be set to {@link #getRequestURL()}
     * plus any {@link #getQueryString()} </li>
     * <li>If {@link HttpServletResponse#addCookie(Cookie)} has been called on
     * the associated response, then a corresponding Cookie header will be added
     * to the PushBuilder, unless the {@link Cookie#getMaxAge()} is &lt;=0, in
     * which case the Cookie will be removed from the builder.</li>
     * <li>If this request has has the conditional headers If-Modified-Since or
     * If-None-Match then the {@link PushBuilderImpl#isConditional()} header is
     * set to true.
     * </ul>
     *
     * <p>
     ...
     * <dl>
     * <dt>org.eclipse.jetty.server.Server</dt><dd>The Jetty Server
     * instance</dd>
     * <dt>org.eclipse.jetty.server.HttpChannel</dt><dd>The HttpChannel for this
     * request</dd>
     * <dt>org.eclipse.jetty.server.HttpConnection</dt><dd>The HttpConnection or
     * null if another transport is used</dd>
     * </dl>
     * While these attributes may look like security problems, they are exposing
     * nothing that is not already available via reflection from a Request
     * instance.
     *
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return The current {@link Context context} used for this request, or
     * <code>null</code> if {@link #setContext} has not yet been called.
     */
 ...
 /*
     ...
     *
     * @return the remote {@link InetSocketAddress} for this request, or null if
     * the request has no remote (see {@link ServletRequest#getRemoteAddr()} for
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return Returns the original uri passed in metadata before
     ...
     * {@link Authentication} is not {@link Authentication.User} (eg.
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return True if this is the first call of <code>takeNewContext()</code>
     * since the last
     * {@link #setContext(org.eclipse.jetty.server.handler.ContextHandler.Context)}
     * call.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the character encoding used for the query string. This call will
     * effect the return of getQueryString and getParamaters. It must be called
     * before any getParameter methods.
     *
     * The request attribute
     * "org.eclipse.jetty.server.server.Request.queryEncoding" may be set as an
     * alternate method of calling setQueryEncoding.
     *
    ...
    public AsyncContext startAsync(ServletRequest servletRequest, ServletResponse servletResponse) throws IllegalStateException {
        ...
        } else // TODO probably need to strip encoded context from requestURI, but will do this for now:
        {
