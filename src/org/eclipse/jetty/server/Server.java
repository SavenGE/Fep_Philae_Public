...
/* ------------------------------------------------------------ */
/**
 * Jetty HTTP Servlet Server. This class is the main class for the Jetty HTTP
 * Servlet server. It aggregates Connectors (HTTP request receivers) and request
 * Handlers. The server is itself a handler and a ThreadPool. Connectors use the
...
public class Server extends HandlerWrapper implements Attributes {
    ...
    /* ------------------------------------------------------------ */
    /**
     * Convenience constructor Creates server and a {@link ServerConnector} at
     * the passed port.
     *
     * @param port The port of a network HTTP connector (or 0 for a randomly
     * allocated port).
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set a graceful stop time. The {@link StatisticsHandler} must be
     * configured so that open connections can be tracked for a graceful
     * shutdown.
     *
     * @see
     * org.eclipse.jetty.util.component.ContainerLifeCycle#setStopTimeout(long)
     */
    @Override
     ...
     *
     * @param stop If true, this server instance will be explicitly stopped when
     * the JVM is shutdown. Otherwise the JVM is stopped with the server
     * running.
    ...
    /* ------------------------------------------------------------ */
    /**
     * Convenience method which calls {@link #getConnectors()} and
     * {@link #setConnectors(Connector[])} to remove a connector.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the connectors for this server. Each connector has this server set as
     * it's ThreadPool and its Handler.
     *
    ...
    /**
     * @param dumpAfterStart true if {@link #dumpStdErr()} is called after
     * starting
     */
    ...
    /**
     * @param dumpBeforeStop true if {@link #dumpStdErr()} is called before
     * stopping
     */
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return The URI of the first {@link NetworkConnector} and first
     * {@link ContextHandler}, or null
     */
    public URI getURI() {
