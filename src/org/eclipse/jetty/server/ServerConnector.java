...
/**
 * This {@link Connector} implementation is the primary connector for the Jetty
 * server over TCP/IP. By the use of various {@link ConnectionFactory} instances
 * it is able to accept connections for HTTP, HTTP/2 and WebSocket, either
 * directly or over SSL.
 * <p>
 * The connector is a fully asynchronous NIO based implementation that by
 * default will use all the commons services (eg
 * {@link Executor}, {@link Scheduler}) of the passed {@link Server} instance,
 * but all services may also be constructor injected into the connector so that
 * it may operate with dedicated or otherwise shared services.
 * </p>
 * <h2>Connection Factories</h2>
 * <p>
 * Various convenience constructors are provided to assist with common
 * configurations of ConnectionFactories, whose generic use is described in
 * {@link AbstractConnector}. If no connection factories are passed, then the
 * connector will default to use a {@link HttpConnectionFactory}. If an non null
 * {@link SslContextFactory} instance is passed, then this used to instantiate a
 * {@link SslConnectionFactory} which is prepended to the other passed or
 * default factories.
 * </p>
 * <h2>Selectors</h2>
 * <p>
 * The connector will use the {@link Executor} service to execute a number of
 * Selector Tasks, which are implemented to each use a NIO {@link Selector}
 * instance to asynchronously schedule a set of accepted connections. It is the
 * selector thread that will call the {@link Callback} instances passed in the
 * {@link EndPoint#fillInterested(Callback)} or
 * {@link EndPoint#write(Callback, java.nio.ByteBuffer...)} methods. It is
 * expected that these callbacks may do some non-blocking IO work, but will
 * always dispatch to the {@link Executor} service any blocking, long running or
 * application tasks.
 * </p>
 * <p>
 * The default number of selectors is equal to half of the number of processors
 * available to the JVM, which should allow optimal performance even if all the
 * connections used are performing significant non-blocking work in the callback
 * tasks.
 * </p>
 */
...
public class ServerConnector extends AbstractNetworkConnector {
     ...
     * {@link HttpConnectionFactory} as the only factory.</p>
     *
     * @param server The {@link Server} this connector will accept connection
     * for.
     */
     ...
     * @param server The {@link Server} this connector will accept connection
     * for.
     * @param acceptors the number of acceptor threads to use, or -1 for a
     * default value. Acceptors accept new TCP/IP connections. If 0, then the
     * selector threads are used to accept connections.
     * @param selectors the number of selector threads, or &lt;=0 for a default
     * value. Selectors notice and schedule established connection that can make
     * IO progress.
     */
    public ServerConnector(
            @Name("server") Server server,
     ...
     * value. Selectors notice and schedule established connection that can make
     * IO progress.
     * @param factories Zero or more {@link ConnectionFactory} instances used to
    ...
    /**
     * <p>
     * Construct a Server Connector with the passed Connection factories.</p>
     *
     * @param server The {@link Server} this connector will accept connection
     * for.
     * @param factories Zero or more {@link ConnectionFactory} instances used to
     ...
     * <p>
     * Construct a ServerConnector with a private instance of
     * {@link HttpConnectionFactory} as the primary protocol</p>.
     *
     * @param server The {@link Server} this connector will accept connection
     * for.
     * @param sslContextFactory If non null, then a {@link SslConnectionFactory}
     ...
     * for.
     * @param sslContextFactory If non null, then a {@link SslConnectionFactory}
     * is instantiated and prepended to the list of ConnectionFactories, with
     * the first factory being the default protocol for the
     ...
     * @param executor An executor used to run tasks for handling requests,
     * acceptors and selectors. If null then use the servers executor
     * @param scheduler A scheduler used to schedule timeouts. If null then use
     * the servers scheduler
     * @param bufferPool A ByteBuffer pool used to allocate buffers. If null
     * then create a private pool with default configuration.
     * @param acceptors the number of acceptor threads to use, or -1 for a
     * default value. Acceptors accept new TCP/IP connections. If 0, then the
     * selector threads are used to accept connections.
     * @param selectors the number of selector threads, or &lt;=0 for a default
    ...
    /**
     * <p>
     * Sets whether this connector uses a channel inherited from the JVM.</p>
     * <p>
     ...
     * used to launch Jetty.</p>
     *
     * @see ServerConnector#openAcceptChannel()
     * @param inheritChannel whether this connector uses a channel inherited
     * from the JVM.
     */
    ...
    /**
     * Open the connector using the passed ServerSocketChannel. This open method
     * can be called before starting the connector to pass it a
     * ServerSocketChannel that will be used instead of one returned from
     * {@link #openAcceptChannel()}
     *
    ...
    /**
     * @param acceptQueueSize the accept queue size (also known as accept
     * backlog)
     */
