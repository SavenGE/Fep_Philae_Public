 ...
 * {@link org.eclipse.jetty.server.HttpOutput.Interceptor} mechanism to buffer
 * the entire response content until the output is closed. This allows the
 * commit to be delayed until the response is complete and thus headers and
 * response status can be changed while writing the body.
 * <p>
 * Note that the decision to buffer is influenced by the headers and status at
 * the first write, and thus subsequent changes to those headers will not
 * influence the decision to buffer or not.
 * <p>
 ...
 /* ------------------------------------------------------------ */
    private class BufferedInterceptor implements HttpOutput.Interceptor {
        ...
        protected void commit(Queue<ByteBuffer> buffers, Callback callback) {
            ...
            } else if (_buffers.size() == 1) // just flush it with the last callback
            {
