 ...
 * plus setting the context classloader.
 *
 * <p>
 * If the context init parameter
 * "org.eclipse.jetty.server.context.ManagedAttributes" is set to a comma
 * separated list of names, then they are treated as context attribute names,
 * which if set as attributes are passed to the servers Container so that they
 * may be managed with JMX.
 * <p>
 * The maximum size of a form that can be processed by this context is
 * controlled by the system properties
 * org.eclipse.jetty.server.Request.maxFormKeys and
 * org.eclipse.jetty.server.Request.maxFormContentSize. These can also be
 * configured with {@link #setMaxFormContentSize(int)} and
 * {@link #setMaxFormKeys(int)}
 * <p>
 * This servers executor is made available via a context attributed
 * "org.eclipse.jetty.server.Executor".
 * <p>
 * By default, the context is created with alias checkers for
 * {@link AllowSymLinkAliasChecker} (unix only) and
 * {@link ApproveNonExistentDirectoryAliases}. If these alias checkers are not
 * required, then {@link #clearAliasChecks()} or {@link #setAliasChecks(List)}
 * should be called.
 */
...
public class ContextHandler extends ScopedHandler implements Attributes, Graceful {
     ...
     * {@link #setManagedAttribute(String, Object)}, which typically initiates
     * the creation of a JMX MBean for the attribute value.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return the allowNullPathInfo true if /context is not redirected to
     * /context/
     */
     ...
     * null host name or null/empty array means any hostname is acceptable. Host
     * names may be String representation of IP addresses. Host names may start
     * with '*.' to wildcard one level of names. Hosts may start with '@', in
     * which case they will match the {@link Connector#getName()} for the
     * request.
     */
     ...
     *
     * @param virtualHosts Array of virtual hosts that this context responds to.
     ...
     * start with '*.' to wildcard one level of names. Host names may start with
     * '@', in which case they will match the {@link Connector#getName()} for
     * the request.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Removes an array of virtual host entries, if this removes all entries the
     * _vhosts will be set to null
     *
     * @param virtualHosts Array of virtual hosts that this context responds to.
     * A null host name or null/empty array means any hostname is acceptable.
     * Host names may be String representation of IP addresses. Host names may
     ...
     * name or empty array means any hostname is acceptable. Host names may be
     * String representation of IP addresses. Host names may start with '*.' to
     * wildcard one level of names.
     */
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set shutdown status. This field allows for graceful shutdown of a
     * context. A started context may be put into non accepting state so that
     * existing requests can complete, but no new requests are accepted.
     *
     */
    @Override
 ...
 /*
     ...
     * {@link ContextHandler#doStart()} instead of a call to super.doStart().
     * This allows derived classes to insert additional handling (Eg
     * configuration) before the call to super.doStart by this method will start
     * contained handlers.
     *
    ...
    public boolean checkVirtualHost(final Request baseRequest) {
        if (_vhosts != null && _vhosts.length > 0) {
            ...
            for (String contextVhost : _vhosts) {
                ...
                switch (c) {
                    case '*':
                        if (contextVhost.startsWith("*.")) // wildcard only at the beginning, and only for one additional subdomain level
                        {
 ...
 /*
    ...
    public boolean checkContext(final String target, final Request baseRequest, final HttpServletResponse response) throws IOException {
        ...
        if (!_allowNullPathInfo && _contextPath.endsWith(URIUtil.SLASH) && _contextPath.length() > 1) {
    ...
    /**
     * @see
     * org.eclipse.jetty.server.handler.ScopedHandler#doScope(java.lang.String,
    ...
    /**
     * @see
     * org.eclipse.jetty.server.handler.ScopedHandler#doHandle(java.lang.String,
     * org.eclipse.jetty.server.Request, javax.servlet.http.HttpServletRequest,
     * javax.servlet.http.HttpServletResponse)
     */
    @Override
     ...
     * @param reason An object that indicates the reason the scope is being
     * entered.
     */
     ...
     *
     * @param request The request to scope the thread to (may be null if no
     * particular request is in scope)
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Check the target. Called by
     * {@link #handle(String, Request, HttpServletRequest, HttpServletResponse)}
     * when a target within a context is determined. If the target is protected,
     * 404 is returned.
     *
 ...
 /*
     ...
     *
     * @param base The resource used as the base for all static content of this
     * context.
     ...
     * context. Any string accepted by {@link Resource#newResource(String)} may
     * be passed and the call is equivalent to
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return The names of the files which the server should consider to be
     * welcome files in this context.
     * @see
     * <a href="http://jcp.org/aboutJava/communityprocess/final/jsr154/index.html">The
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the maximum size of a form post, to protect against DOS attacks from
     * large forms.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the maximum number of form Keys to protect against DOS attack from
     * crafted hash keys.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param compactPath True if URLs are compacted to replace multiple '/'s
     * with a single '/'
     */
     ...
     *
     * @return a map of all the locale encodings: key is name of the locale and
     * value is the char encoding
     */
 ...
 /*
     ...
     * Context.
     * <p>
     * A partial implementation of {@link javax.servlet.ServletContext}. A
     * complete implementation is provided by the derived
     * {@link ContextHandler}.
     * </p>
     *
     *
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Approve Aliases of a non existent directory. If a directory "/foobar/"
     * does not exist, then the resource is aliased to "/foobar". Accept such
     * aliases.
     */
    ...
    /**
     * Listener for all threads entering context scope, including async IO
     * callbacks
     */
