 ...
 * {@link ContextHandler#setErrorHandler(ErrorHandler)} or
 * {@link Server#setErrorHandler(ErrorHandler)}. It is called by the
 * HttpResponse.sendError method to write a error page via
 * {@link #handle(String, Request, HttpServletRequest, HttpServletResponse)} or
 * via {@link #badMessageError(int, String, HttpFields)} for bad requests for
 * which a dispatch cannot be done.
 *
 */
public class ErrorHandler extends AbstractHandler {
 ...
 /*
     ...
     * acceptable to the user-agent. The Accept header is evaluated in quality
     * order and the method
     * {@link #generateAcceptableResponse(Request, HttpServletRequest, HttpServletResponse, int, String, String)}
     * is called for each mimetype until {@link Request#isHandled()} is
     * true.</p>
     *
     ...
     * <p>
     * Uses the user-agent's <code>Accept-Charset</code> to get response
     * {@link Writer}. The acceptable charsets are tested in quality order if
     * they are known to the JVM and the first known is set on
     ...
     * <code>Accept</code> header, until {@link Request#isHandled()} is true and
     * a response of the appropriate type is generated.
     *
     * @param baseRequest The base request
     * @param request The servlet request (may be wrapped)
     * @param response The response (may be wrapped)
     * @param code the http error code
     * @param message the http error message
     * @param mimeType The mimetype to generate (may be *&#47;*or other
     * wildcard)
     ...
     * <p>
     * Generate a error response body to be sent for a bad message. In this case
     * there is something wrong with the request, so either a request cannot be
     * built, or it is not safe to build a request. This method allows for a
     * simple error page body to be returned and some response headers to be
     * set.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param showMessageInTitle if true, the error message appears in page
     * title
     */
