 ...
 * provided by white/black lists that include both internet addresses and URIs.
 * This handler uses the real internet address of the connection, not one
 * reported in the forwarded for headers, as this cannot be as easily forged.
 * <p>
 * Typically, the black/white lists will be used in one of three modes:
 * <ul>
 * <li>Blocking a few specific IPs/URLs by specifying several black list
 * entries.
 * <li>Allowing only some specific IPs/URLs by specifying several white lists
 * entries.
 * <li>Allowing a general range of IPs/URLs by specifying several general white
 * list entries, that are then further refined by several specific black list
 * exceptions
 * </ul>
 * <p>
 * By default an empty white list is treated as match all. If there is at least
 * one entry in the white list, then a request must match a white list entry.
 * Black list entries are always applied, so that even if an entry matches the
 * white list, a black list entry will override it.
 * <p>
 * You can change white list policy setting whiteListByPath to true. In this
 * mode a request will be white listed IF it has a matching URL in the white
 * list, otherwise the black list applies, e.g. in default mode when
 * whiteListByPath = false and wl = "127.0.0.1|/foo", /bar request from
 * 127.0.0.1 will be blacklisted, if whiteListByPath=true then not.
 * <p>
 * Internet addresses may be specified as absolute address or as a combination
 * of four octet wildcard specifications (a.b.c.d) that are defined as follows.
 * </p>
 * <pre>
 ...
 * </pre>
 * <p>
 * Internet address specification is separated from the URI pattern using the
 * "|" (pipe) character. URI patterns follow the servlet specification for
 * simple * prefix and suffix wild cards (e.g. /, /foo, /foo/bar, /foo/bar/*,
 * *.baz).
 * <p>
 * Earlier versions of the handler used internet address prefix wildcard
 * specification to define a range of the internet addresses (e.g. 127., 10.10.,
 * 172.16.1.). They also used the first "/" character of the URI pattern to
 * separate it from the internet address. Both of these features have been
 ...
 * <li>10.10.1.2|/foo/* - all requests from IP 10.10.1.2 to URIs starting with
 * /foo/
 * <li>10.10.1.2|*.html - all requests from IP 10.10.1.2 to URIs ending with
 * .html
 * <li>10.10.0-255.0-255 - all requests from IPs within 10.10.0.0/16 subnet
 * <li>10.10.0-.-255|/foo/bar - all requests from IPs within 10.10.0.0/16 subnet
 * to URI /foo/bar
 * <li>10.10.0-3,1,3,7,15|/foo/* - all requests from IPs addresses with last
 * octet equal to 1,3,7,15 in subnet 10.10.0.0/22 to URIs starting with /foo/
 * </ul>
 * <p>
 * Earlier versions of the handler used internet address prefix wildcard
 * specification to define a range of the internet addresses (e.g. 127., 10.10.,
...
public class IPAccessHandler extends HandlerWrapper {
    ...
    protected void add(String entry, PathMap<IPAddressMap<Boolean>> patternMap) {
        if (entry != null && entry.length() > 0) {
            ...
            if (addr != null && !"".equals(addr)) // MUST NOT BE null
            {
