 ...
 * ScopedHandler.
 *
 * A ScopedHandler is a HandlerWrapper where the wrapped handlers each define a
 * scope.
 *
 * <p>
 * When
 * {@link #handle(String, Request, HttpServletRequest, HttpServletResponse)} is
 * called on the first ScopedHandler in a chain of HandlerWrappers, the
 * {@link #doScope(String, Request, HttpServletRequest, HttpServletResponse)}
 * method is called on all contained ScopedHandlers, before the
 * {@link #doHandle(String, Request, HttpServletRequest, HttpServletResponse)}
 * method is called on all contained handlers.</p>
 *
 * <p>
 * For example if Scoped handlers A, B &amp; C were chained together, then the
 * calling order would be:</p>
 * <pre>
 * A.handle(...)
 *   A.doScope(...)
 *     B.doScope(...)
 ...
 *
 * <p>
 * If non scoped handler X was in the chained A, B, X &amp; C, then the calling
 * order would be:</p>
 * <pre>
 * A.handle(...)
 *   A.doScope(...)
...
public abstract class ScopedHandler extends HandlerWrapper {
     ...
     * Scope the handler
     * <p>
     * Derived implementations should call
     ...
     * processing
     * @throws ServletException if unable to handle the request or response due
     * to underlying servlet issue
     */
     ...
     * <p>
     * Derived implementations should call
     * {@link #nextHandle(String, Request, HttpServletRequest, HttpServletResponse)}
     *
     * @param target The target of the request - either a URI or a name.
     * @param baseRequest The original unwrapped request object.
     * @param request The request either as the {@link Request} object or a
