...
/* ------------------------------------------------------------ */
/**
 * A handler that shuts the server down on a valid request. Used to do "soft"
 * restarts from Java. If _exitJvm is set to true a hard System.exit() call is
 * being made. If _sendShutdownAtStart is set to true, starting the server will
 * try to shut down an existing server at the same port. If _sendShutdownAtStart
 * is set to true, make a http call to "http://localhost:" + port +
 * "/shutdown?token=" + shutdownCookie in order to shut down the server.
 *
 * This handler is a contribution from Johannes Brodwall:
...
public class ShutdownHandler extends HandlerWrapper {
    ...
    /**
     * Creates a listener that lets the server be shut down remotely (but only
     * from localhost).
     *
     * @param server the Jetty instance that should be shut down
     * @param shutdownToken a secret password to avoid unauthorized shutdown
     * attempts
     */
    @Deprecated
     ...
     * @param shutdownToken a secret password to avoid unauthorized shutdown
     * attempts
     * @param exitJVM If true, when the shutdown is executed, the handler class
     * System.exit()
