...
/**
 * A Handler that can dynamically GZIP uncompress requests, and compress
 * responses.
 * <p>
 * The GzipHandler can be applied to the entire server (a {@code gzip.mod} is
 * included in the {@code jetty-home}) or it may be applied to individual
 * contexts.
 * </p>
 * <p>
 * Both Request uncompress and Response compress are gated by a configurable
 * {@link DispatcherType} check on the GzipHandler. (This is similar in behavior
 * to a {@link javax.servlet.Filter} configuration you would find in a Servlet
 * Descriptor file ({@code WEB-INF/web.xml})
 * <br>(Default: {@link DispatcherType#REQUEST}).
 * </p>
 * <p>
 * Requests with a {@code Content-Encoding} header with the value {@code gzip}
 * will be uncompressed by a {@link GzipHttpInputInterceptor} for any API that
 * uses {@link HttpServletRequest#getInputStream()} or
 * {@link HttpServletRequest#getReader()}.
 * </p>
 * <p>
 * Response compression has a number of checks before GzipHandler will perform
 * compression.
 * </p>
 * <ol>
 * <li>
 ...
 * </li>
 * <li>
 * Is the {@link HttpServletRequest#getMethod()} allowed by the configured HTTP
 * Method Filter.
 * <br> (Default: {@code GET})
 * </li>
 * <li>
 ...
 * </li>
 * <li>
 * Is the Response {@code Content-Length} header present, and does its value
 * meet the minimum gzip size requirements?
 * <br> (Default: 16 bytes. see {@link GzipHandler#DEFAULT_MIN_GZIP_SIZE})
 * </li>
 * <li>
 * Is the Request {@code Accept} header present and does it contain the required
 * {@code gzip} value?
 * </li>
 * </ol>
 * <p>
 * When you encounter a configurable filter in the GzipHandler (method, paths,
 * user-agent, mime-types, etc) that has both Included and Excluded values, note
 * that the Included values always win over the Excluded values.
 * </p>
 * <p>
 * <em>Important note about Default Values</em>: It is important to note that
 * the GzipHandler will automatically configure itself from the MimeType present
 * on the Server, System, and Contexts and the ultimate set of default values
 * for the various filters (paths, methods, mime-types, etc) can be influenced
 * by the available mime types to work with.
 * </p>
 * <p>
 * ETag (or Entity Tag) information: any Request headers for
 * {@code If-None-Match} or {@code If-Match} will be evaluated by the
 * GzipHandler to determine if it was involved in compression of the response
 * earlier. This is usually present as a {@code --gzip} suffix on the ETag that
 * the Client User-Agent is tracking and handed to the Jetty server. The special
 * {@code --gzip} suffix on the ETag is how GzipHandler knows that the content
 * passed through itself, and this suffix will be stripped from the Request
 * header values before the request is sent onwards to the specific webapp /
 * servlet endpoint for handling. If a ETag is present in the Response headers,
 * and GzipHandler is compressing the contents, it will add the {@code --gzip}
 ...
 * {@link org.eclipse.jetty.server.HttpOutput.Interceptor} mechanism to allow
 * for effective and efficient compression of the response on all Output API
 * usages:
 * </p>
 * <ul>
 * <li> {@link javax.servlet.ServletOutputStream} - Obtained from
 * {@link HttpServletResponse#getOutputStream()} using the traditional Blocking
 * I/O techniques
 * </li>
 * <li> {@link javax.servlet.WriteListener} - Provided to
 ...
 * </li>
 * <li> {@link java.io.PrintWriter} - Obtained from
 * {@link HttpServletResponse#getWriter()} using Blocking I/O techniques
 * </li>
 * </ul>
 * <p>
 * Historically the compression of responses were accomplished via Servlet
 * Filters (eg: {@code GzipFilter}) and usage of
 * {@link javax.servlet.http.HttpServletResponseWrapper}. Since the introduction
 * of Async I/O in Servlet 3.1, this older form of Gzip support in web
 * applications has been problematic and bug ridden.
 * </p>
 */
public class GzipHandler extends HandlerWrapper implements GzipFactory {
     ...
     *
     * @param dispatchers the set of {@link DispatcherType} that this filter
     * will operate on
     */
    ...
    /**
     * Set the list of supported {@link DispatcherType} that this filter will
     * operate on.
     *
     * @param dispatchers the list of {@link DispatcherType} that this filter
     * will operate on
     */
     ...
     * For backward compatibility the MIME types parameters may be comma
     * separated strings, but this will not be supported in future versions of
     * Jetty.
     * </p>
     *
     * @param types The mime types to exclude (without charset or other
     * parameters).
     ...
     *
     * <p>
     * There are 2 syntaxes supported, Servlet <code>url-pattern</code> based,
     * and Regex based. This means that the initial characters on the path spec
     * line are very strict, and determine the behavior of the path matching.
     * <ul>
     * <li>If the spec starts with <code>'^'</code> the spec is assumed to be a
     * regex based path spec and will match with normal Java regex rules.</li>
     * <li>If the spec starts with <code>'/'</code> then spec is assumed to be a
     * Servlet url-pattern rules path spec for either an exact match or prefix
     * based match.</li>
     ...
     * For backward compatibility the pathspecs may be comma separated strings,
     * but this will not be supported in future versions.
     ...
     *
     * @return True if {@link Deflater#SYNC_FLUSH} is used, else
     * {@link Deflater#NO_FLUSH}
     ...
     *
     * @param syncFlush True if {@link Deflater#SYNC_FLUSH} is used, else
     * {@link Deflater#NO_FLUSH}
     * @see #isSyncFlush()
     */
     ...
     *
     * @param types The mime types to include (without charset or other
     * parameters) For backward compatibility the mimetypes may be comma
     * separated strings, but this will not be supported in future versions.
     ...
     *
     * @return the filter list of included HTTP methods
     * @deprecated use {@link #getIncludedMethods()} instead. (Will be removed
     * in Jetty 10)
     */
    @Deprecated
    ...
    /**
     * Get the minimum size, in bytes, that a response {@code Content-Length}
     * must be before compression will trigger.
     *
    ...
    /**
     * Get the size (in bytes) of the {@link java.util.zip.Inflater} buffer used
     * to inflate compressed requests.
     *
     ...
     * to inflate comrpessed requests.
     *
    ...
    /**
     * Test if the provided User-Agent is allowed based on the User-Agent
     * filters.
     *
    ...
    /**
     * Test if the provided Request URI is allowed based on the Path Specs
     * filters.
     *
     ...
     *
     * @param checkGzExists whether to check if a static gz file exists for the
     * resource that the DefaultServlet may serve as precompressed.
     * @deprecated feature will be removed in Jetty 10.x, with no replacement.
     */
    @Deprecated
     ...
     *
     * @param compressionLevel The compression level to use to initialize
    ...
    /**
     * Set the excluded filter list of User-Agent patterns (replacing any
     * previously set)
     *
    ...
    /**
     * Set the excluded filter list of HTTP methods (replacing any previously
     * set)
     *
    ...
    /**
     * Set the included filter list of User-Agent patterns (replacing any
     * previously set)
     *
     * @param patterns Regular expressions matching user agents to include
    ...
    /**
     * Set the included filter list of HTTP methods (replacing any previously
     * set)
     *
