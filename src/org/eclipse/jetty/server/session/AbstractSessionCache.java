 ...
 *
 * A base implementation of the {@link SessionCache} interface for managing a
 * set of Session objects pertaining to a context in memory.
 *
 * This implementation ensures that multiple requests for the same session id
 * always return the same Session object.
 *
 * It will delay writing out a session to the SessionDataStore until the last
 * request exits the session. If the SessionDataStore supports passivation then
 * the session passivation and activation listeners are called appropriately as
 * the session is written.
 *
 * This implementation also supports evicting idle Session objects. An idle
 * Session is one that is still valid, has not expired, but has not been
 * accessed by a request for a configurable amount of time. An idle session will
 * be first passivated before it is evicted from the cache.
 *
 */
@ManagedObject
public abstract class AbstractSessionCache extends ContainerLifeCycle implements SessionCache {
    ...
    /**
     * If true, as soon as a new session is created, it will be persisted to the
     * SessionDataStore
     */
    protected boolean _saveOnCreate = false;

    /**
     * If true, a session that will be evicted from the cache because it has
     * been inactive too long will be saved before being evicted.
     */
    protected boolean _saveOnInactiveEviction;

    /**
     * If true, a Session whose data cannot be read will be deleted from the
     * SessionDataStore.
     */
     ...
     * @return null if the session wasn't already in the map, or the existing
     * entry otherwise
     */
    ...
    /**
     * @see
     * org.eclipse.jetty.server.session.SessionCache#initialize(org.eclipse.jetty.server.session.SessionContext)
     */
    @Override
    ...
    /**
     * @see
     * org.eclipse.jetty.server.session.SessionCache#setSessionDataStore(org.eclipse.jetty.server.session.SessionDataStore)
     */
    @Override
    ...
    /**
     * -1 means we never evict inactive sessions. 0 means we evict a session
     * after the last request for it exits &gt;0 is the number of seconds after
     * which we evict inactive sessions from the cache
     *
     ...
     *
     * @param removeUnloadableSessions if <code>true</code> unloadable sessions
     * will be removed from session store
     */
    @Override
     ...
     *
     * If the session object is not in this session store, try getting the data
     * for it from a SessionDataStore associated with the session manager.
     *
     ...
     *
     * @param id the id
     * @return a Session object filled with data or null if the session doesn't
     * exist
     * @throws Exception
     */
     ...
     *
     * This should be called when a request exists the session. Only when the
     * last simultaneous request exists the session will any action be taken.
     *
     * If there is a SessionDataStore write the session data through to it.
     *
     * If the SessionDataStore supports passivation, call the passivate/active
     * listeners.
     *
     * If the evictionPolicy == SessionCache.EVICT_ON_SESSION_EXIT then after we
     * have saved the session, we evict it from the cache.
     *
     * @see org.eclipse.jetty.server.session.SessionCache#put(java.lang.String,
     ...
     *
     * This method will first check with the object store. If it doesn't exist
     * in the object store (might be passivated etc), it will check with the
     * data store.
     *
     * @throws Exception the Exception
     *
     * @see
     * org.eclipse.jetty.server.session.SessionCache#exists(java.lang.String)
     */
    @Override
     ...
     *
     * @see
     * org.eclipse.jetty.server.session.SessionCache#contains(java.lang.String)
     */
    @Override
     ...
     *
     * @see
     * org.eclipse.jetty.server.session.SessionCache#delete(java.lang.String)
     */
    @Override
    ...
    /**
     * Check a session for being inactive and thus being able to be evicted, if
     * eviction is enabled.
     *
     *
    ...
    /**
     * @see
     * org.eclipse.jetty.server.session.SessionCache#renewSessionId(java.lang.String,
     * java.lang.String)
     */
    @Override
    ...
    /**
     * @see
     * org.eclipse.jetty.server.session.SessionCache#setSaveOnInactiveEviction(boolean)
     */
    @Override
    ...
    /**
     * Whether we should save a session that has been inactive before we boot it
     * from the cache.
     *
    ...
    /**
     * @see
     * org.eclipse.jetty.server.session.SessionCache#newSession(javax.servlet.http.HttpServletRequest,
