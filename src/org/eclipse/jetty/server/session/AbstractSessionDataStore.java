...
public abstract class AbstractSessionDataStore extends ContainerLifeCycle implements SessionDataStore {
    ...
    /**
     * Implemented by subclasses to resolve which sessions this node should
     * attempt to expire.
     *
     * @param candidates the ids of sessions the SessionDataStore thinks has
     * expired
     * @return the reconciled set of session ids that this node should attempt
     * to expire
     */
    ...
    /**
     * @see
     * org.eclipse.jetty.server.session.SessionDataStore#getExpired(java.util.Set)
     */
    @Override
    ...
    /**
     * The minimum time in seconds between save operations. Saves normally occur
     * every time the last request exits as session. If nothing changes on the
     * session except for the access time and the persistence technology is
     * slow, this can cause delays.
     * <p>
     * By default the value is 0, which means we save after the last request
     * exists. A non zero value means that we will skip doing the save if the
     * session isn't dirty if the elapsed time since the session was last saved
