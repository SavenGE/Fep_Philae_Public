 ...
 *
 * Ideally, multiple requests for the same session id in the same context will
 * always share the same Session object from the SessionCache, but it would be
 * possible for implementations of SessionCache to create a fresh object for
 * each request.
 *
 * The SessionData pertaining to the Session objects is obtained from/written to
 * a SessionDataStore. The SessionDataStore is the authoritative source of
 * session data:
 * <ul>
 * <li>if the session data is not present in the SessionDataStore the session
 * does not exist.</li>
 * <li>if the session data is present in the SessionDataStore but its expiry
 * time has passed then the session is deemed to have expired and is therefore
 * invalid</li>
 * </ul>
 *
 ...
 * <li>whenever the Session has not been accessed for a configurable number of
 * seconds</li>
 * </ul>.
 *
 * Eviction can save memory, and can also help mitigate some of the problems of
 * a non-sticky load balancer by forcing the session data to be re-read from the
 * SessionDataStore more frequently.
 *
 */
public interface SessionCache extends LifeCycle {
    ...
    /**
     * Finish using a Session. This is called by the SessionHandler once a
     * request is finished with a Session. SessionCache implementations may want
     * to delay writing out Session contents until the last request exits a
     * Session.
     *
     * @param id the session id
    ...
    /**
     * Check to see if a Session is in the cache. Does NOT consult the
     * SessionDataStore.
     *
     * @param id the session id
     * @return true if a Session object matching the id is present in the cache,
     * false otherwise
     * @throws Exception if any error occurred
     */
    ...
    /**
     * Remove a Session completely: from both this cache and the
     * SessionDataStore.
     *
     * @param id the session id
     ...
     * The Session in the cache should be checked, but also the
     * SessionDataStore, as that is the authoritative source of all session
     * information.
     *
     * @param candidates the session ids to check
     * @return the set of session ids that have actually expired: this can be a
     * superset of the original candidate list.
     */
    ...
    /**
     * A SessionDataStore that is the authoritative source of session
     * information.
     *
     ...
     * </ul>
     *
     * @param policy -1 is never evict; 0 is evict-on-exit; and any other
     * positive value is the time in seconds that a session can be idle before
     * it can be evicted.
     */
    ...
    /**
     * Whether or not a a session that is about to be evicted should be saved
     * before being evicted.
     *
     * @param saveOnEvict <code>true</code> if the session should be saved
     * before being evicted
     */
    ...
    /**
     *
     * @return <code>true</code> if the session should be saved before being
     * evicted
     */
    boolean isSaveOnInactiveEviction();

    /**
     * Whether or not a session that is newly created should be immediately
     * saved. If false, a session that is created and invalidated within a
     * single request is never persisted.
     *
     * @param saveOnCreate <code>true</code> to immediately save the newly
     * created session
     */
    ...
    /**
     *
     * @return if <code>true</code> the newly created session will be saved
     * immediately
     */
    boolean isSaveOnCreate();

    /**
     * If the data for a session exists but is unreadable, the SessionCache can
     * instruct the SessionDataStore to delete it.
     *
     * @param removeUnloadableSessions <code>true</code> to delete session which
     * cannot be loaded
     */
