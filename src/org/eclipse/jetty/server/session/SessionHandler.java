...
public class SessionHandler extends ScopedHandler {
    ...
    /* ------------------------------------------------------------ */
    /**
     * Session cookie name. Defaults to <code>JSESSIONID</code>, but can be set
     * with the <code>org.eclipse.jetty.servlet.SessionCookie</code> context
     * init parameter.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Session id path parameter name. Defaults to <code>jsessionid</code>, but
     * can be set with the
     * <code>org.eclipse.jetty.servlet.SessionIdPathParameterName</code> context
     * init parameter. If context init param is "none", or
     * setSessionIdPathParameterName is called with null or "none", no URL
    ...
    public Set<SessionTrackingMode> __defaultSessionTrackingModes
            = Collections.unmodifiableSet(
                    new HashSet<>(
                        ...
                        SessionTrackingMode.COOKIE, SessionTrackingMode.URL
                    })));
    ...
    /**
     * Web.xml session-timeout is set in minutes, but is stored as an int in
     * seconds by HttpSession and the sessionmanager. Thus MAX_INT is the max
     * number of seconds that can be set, and MAX_INT/60 is the max number of
     * minutes that you can set.
     */
     ...
     *
     * Used to ensure that a request for which async has been started has its
     * session completed as the request exits the context.
     */
     ...
     *
     * @param listener the session event listener to add Individual
     * SessionManagers implementations may accept arbitrary listener types, but
     * they are expected to at least handle HttpSessionActivationListener,
 ...
 /*
     ...
     * @return the <code>HttpSession</code> with the corresponding id or null if
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return same as SessionCookieConfig.getSecure(). If true, session cookies
     * are ALWAYS marked as secure. If false, a session cookie is ONLY marked as
     * secure if _secureRequestOnly == true and it is a HTTPS request.
     */
     ...
     * SessionCookieConfig.setSecure(true), in which case the session cookie
     * will be marked as secure on both HTTPS and HTTP.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * A session cookie is marked as secure IFF any of the following conditions
     * are true:
     * <ol>
     ...
     * even though the request that initiated the session came over HTTP, is to
     * support a topology where the web container is front-ended by an SSL
     * offloading load balancer. In this case, the traffic between the client
     ...
     * Servlet Spec 3.0 default behavior when
     * SessionCookieConfig.setSecure==false, which is:
     * <cite>
     * "they shall be marked as secure only if the request that initiated the
     * corresponding session was also secure"
     * </cite>
     * <p>
     * The default for _secureRequestOnly is true, which gives the above
     * behavior. If you set it to false, then a session cookie is NEVER marked
     * as secure, even if the initiating request was secure.
     *
     ...
     * client will only send the cookie value when requesting resources under
     * this path.
     ...
     * method will return a new {@link Cookie cookie object} that should be set
     * on the client in order to link future HTTP requests with the
     ...
     * by default ";" + sessionIdParameterName + "=", for easier lookup in URL
     * strings.
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param metaManager The metaManager used for cross context session
     * management.
     */
     ...
     *
     * @param param the URL path parameter name for session id URL rewriting
     * (null or "none" for no rewriting).
     * @see #getSessionIdPathParameterName()
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return true if the cluster node id (worker id) is returned as part of
     * the session id by {@link HttpSession#getId()}. Default is false.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param nodeIdInSessionId true if the cluster node id (worker id) will be
     * returned as part of the session id by {@link HttpSession#getId()}.
     * Default is false.
     */
     ...
     * {@link HttpSessionListener#sessionDestroyed(HttpSessionEvent)} and
    ...
    /* ------------------------------------------------------------ */
    /**
     * Called periodically by the HouseKeeper to handle the list of sessions
     * that have expired since the last call to scavenge.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Each session has a timer that is configured to go off when either the
     * session has not been accessed for a configurable amount of time, or the
