 ...
 * <p>
 * Maintains a set of included and excluded elements. The method
 * {@link #test(Object)} will return true IFF the passed object is not in the
 * excluded set AND ( either the included set is empty OR the object is in the
 * included set)
 * <p>
...
public class IncludeExcludeSet<T, P> implements Predicate<P> {
     ...
     *
     * @param setClass The type of {@link Set} to using internally to hold
     * patterns. Two instances will be created. one for include patterns and one
     * for exclude patters. If the class is also a {@link Predicate}, then it is
     * also used as the item test for the set, otherwise a
     * {@link SetContainsPredicate} instance is created.
     ...
     * @param includePredicate the Predicate for included item testing (null for
     * simple {@link Set#contains(Object)} test)
     * @param excludeSet the Set of items that represent the excluded space
     * @param excludePredicate the Predicate for excluded item testing (null for
     * simple {@link Set#contains(Object)} test)
     * @param <SET> The type of a set to use as the backing store
     */
     ...
     * @return Boolean.TRUE if t is included, Boolean.FALSE if t is excluded and
     * null if neither
     */
