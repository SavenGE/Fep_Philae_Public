 ...
 * <p>
 * Allows for configured Excludes and Includes using
 * {@link FileSystem#getPathMatcher(String)} syntax.
 * <p>
...
public class PathWatcher extends AbstractLifeCycle implements Runnable {
    ...
    public static class Config implements Predicate<Path> {
         ...
         * Add an exclude PathMatcher.
         * <p>
         * Note: this pattern is FileSystem specific (so use "/" for Linux and
         * OSX, and "\\" for Windows)
         *
         * @param syntaxAndPattern the PathMatcher syntax and pattern to use
         * @see FileSystem#getPathMatcher(String) for detail on syntax and
         * pattern
         */
        ...
        /**
         * Add a <code>glob:</code> syntax pattern exclude reference in a
         * directory relative, os neutral, pattern.
         *
         * <pre>
         *    On Linux:
         ...
         *
         * @param syntaxAndPatterns the list of PathMatcher syntax and patterns
         * to use
         * @see FileSystem#getPathMatcher(String) for detail on syntax and
         * pattern
         */
        ...
        /**
         * Add a <code>glob:</code> syntax pattern reference in a directory
         * relative, os neutral, pattern.
         *
         * <pre>
         *    On Linux:
         *    Config config = new Config(Path("/home/user/example"));
         ...
         *
         * @param dir the directory to build new Config from (using this config
         * as source of includes/excludes)
         ...
         * <p>
         * -999 indicates arbitrarily deep recursion, 0 indicates no recursion,
         * 1 is only one directory deep, and so on.
         *
    ...
    /**
     * Update Quiet Time - set to 1000 ms as default (a lower value in Windows
     * is not supported)
     */
    ...
    /**
     * Request watch on a the given path (either file or dir) using all Config
     * defaults. In the case of a dir, the default is not to recurse into
     * subdirs for watching.
     *
    ...
    public void watch(final Path file) {
        ...
        if (config == null) {
            ...
            watch(config);
        } else //add the include for the file
        {
            config.addIncludeGlobRelative(file.getFileName().toString());
        }
    }
    ...
    /**
     * Create a fresh WatchService and determine if it is a native
    ...
    /**
     * Check to see if the watcher is in a state where it should generate watch
     * events to the listeners. Used to determine if watcher should generate
     ...
     * Subsequent events that arrive for the same path during this quiet time
     * are accumulated and the timer reset. Only when the quiet time has expired
     * are the accumulated events sent. MODIFY events are handled slightly
     * differently - multiple MODIFY events arriving within a quiet time are
     * coalesced into a single MODIFY event. Both the accumulation of events and
     * coalescing of MODIFY events reduce the number and frequency of event
     * reporting for "noisy" files (ie those that are undergoing rapid change).
     *
    ...
    /**
     * Add an event reported by the WatchService to list of pending events that
     * will be sent after their quiet time has expired.
     *
