...
public class SharedBlockingCallback {
     ...
     * been called OK.
     * <p>
     * Implements {@link Callback} because calls to this callback do not blocak,
     * rather they wakeup the thread that is blocked in {@link #block()}
     */
    public class Blocker implements Callback, Closeable {
        ...
        public void failed(Throwable cause) {
            ...
            try {
                if (_state == null) {
                    if (cause == null) {
                        _state = FAILED;
                    } else if (cause instanceof BlockerTimeoutException) // Not this blockers timeout
                    {
         ...
         * a blocking API.
         *
         * @throws IOException if exception was caught during blocking, or
        ...
        public void close() {
            ...
            try {
                ...
                try {
                    ...
                    if (_state instanceof BlockerTimeoutException) // and create a new Blocker
                    {
                        _blocker = new Blocker();
                    } else // else reuse Blocker
                    {
                        _state = IDLE;
                    }
                    _idle.signalAll();
