 ...
 * <p>
 * The sort algorithm works by recursively visiting every item, once and only
 * once. On each visit, the items dependencies are first visited and then the
 ...
 *
 * @param <T> The type to be sorted. It must be able to be added to a
 * {@link HashSet}
 */
public class TopologicalSort<T> {
     ...
     *
     * @param dependent The dependent item will be sorted after all its
     * dependencies
     * @param dependency The dependency item, will be sorted before its
     * dependent item
     */
    ...
    private void visit(T item, Set<T> visited, List<T> sorted, Comparator<T> comparator) {
        ...
        if (!visited.contains(item)) {
            ...
            sorted.add(item);
        } else if (!sorted.contains(item)) // If we have already visited an item, but it has not yet been put in the
    ...
    /**
     * A comparator that is used to sort dependencies in the order they were in
     * the original list. This ensures that dependencies are visited in the
     * original order and no needless reordering takes place.
     *
     * @param <T>
     */
