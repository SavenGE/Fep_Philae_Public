 ...
 * <p>
 * A Jetty {@link Logger} that sends all logs to STDERR ({@link System#err})
 ...
 * logging events that it can generate, including DEBUG, INFO, WARN (and even
 * special internally ignored exception cases).</dd>
 * </dl>
 * </dd>
 *
 ...
 * Name must be a fully qualified class name (package name hierarchy is not
 * supported by this configurable)<br>
 ...
 * <dd>Special Global Configuration, attempt to print the java source file name
 * and line number where the logging event originated from.<br>
 * Default: false
 * </dd>
 *
 * <dt>org.eclipse.jetty.util.log.stderr.LONG=(true|false)</dt>
 * <dd>Special Global Configuration, when true, output logging events to STDERR
 * using long form, fully qualified class names. when false, use abbreviated
 * package names<br>
 * Default: false
 * </dd>
 * <dt>org.eclipse.jetty.util.log.stderr.ESCAPE=(true|false)</dt>
 * <dd>Global Configuration, when true output logging events to STDERR are
 * always escaped so that control characters are replaced with '?"; '\r' with
 * '&lt;' and '\n' replaced '|'<br>
 * Default: true
 * </dd>
 * </dl>
 */
@ManagedObject("Jetty APLog Logging Implementation")
public class APLog extends AbstractLogger {

    private static final String EOL = System.getProperty("line.separator");
    // Do not change output format lightly, people rely on this output format now.
    private static int __tagpad = Integer.parseInt(Log.__props.getProperty("org.eclipse.jetty.util.log.APLog.TAG_PAD", "0"));
    ...
    /**
     * Obtain a APLog reference for the specified class, a convenience method
     * used most often during testing to allow for control over a specific
     * logger.
     * <p>
     * Must be actively using APLog as the Logger implementation.
     *
     * @param clazz the Class reference for the logger to use.
     * @return the APLog logger
     * @throws RuntimeException if APLog is not the active Logger
     * implementation.
     */
    public static APLog getLogger(Class<?> clazz) {
        Logger log = Log.getLogger(clazz);
        if (log instanceof APLog) {
            return (APLog) log;
        }
        throw new RuntimeException("Logger for " + clazz + " is not of type APLog");
    }

    /**
     * Construct an anonymous APLog (no name).
     * <p>
    ...
    /**
     * Construct a named APLog using the {@link Log} defined properties
     *
     * @param name the name of the logger
     */
    public APLog(String name) {
        this(name, null);
    }

    /**
     * Construct a named Logger using the provided properties to configure
     * logger.
     *
     * @param name the name of the logger
     * @param props the configuration properties
     */
    public APLog(String name, Properties props) {
     ...
     *
     * @return true if the class, method, file and line number of a log is
     * logged.
     */
     ...
     *
     * @param source true if the class, method, file and line number of a log is
     * logged.
     */
    ...
    public void warn(String msg, Object... args) {
        if (_level <= LEVEL_WARN) {
            ...
            APMain.acxLog.logEvent(buffer.toString());
        }
    }
    ...
    public void setDebugEnabled(boolean enabled) {
        if (enabled) {
            ...
            for (Logger log : Log.getLoggers().values()) {
                if (log.getName().startsWith(getName()) && log instanceof APLog) {
                    ((APLog) log).setLevel(LEVEL_DEBUG);
                }
            }
        } else {
            ...
            for (Logger log : Log.getLoggers().values()) {
                if (log.getName().startsWith(getName()) && log instanceof APLog) {
                    ((APLog) log).setLevel(((APLog) log)._configuredLevel);
                }
            }
        }
    }
     ...
     * <p>
     * Available values ({@link APLog#LEVEL_ALL}, {@link APLog#LEVEL_DEBUG}, {@link APLog#LEVEL_INFO},
     * {@link APLog#LEVEL_WARN})
     *
...
//        tag(buffer, d, ms, level);
    ...
    private void tag(StringBuilder buffer, String d, int ms, String tag) {
        ...
        if (_source) {
            ...
            for (int i = 0; i < frames.length; i++) {
                ...
                if (clazz.equals(APLog.class.getName()) || clazz.equals(Log.class.getName())) {
                    continue;
                }
    ...
    protected Logger newLogger(String fullname) {
        APLog logger = new APLog(fullname);
