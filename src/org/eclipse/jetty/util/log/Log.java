...
/**
 * Logging. This class provides a static logging interface. If an instance of
 * the org.slf4j.Logger class is found on the classpath, the static log methods
 * are directed to a slf4j logger for "org.eclipse.log". Otherwise the logs are
 * directed to stderr.
 * <p>
 * The "org.eclipse.jetty.util.log.class" system property can be used to select
 * a specific logging implementation.
 * <p>
 * If the system property org.eclipse.jetty.util.log.IGNORED is set, then
 * ignored exceptions are logged in detail.
 *
 * @see StdErrLog
 * @see Slf4jLog
 */
public class Log {
    ...
    /**
     * Legacy flag indicating if {@link Logger#ignore(Throwable)} methods
     * produce any output in the {@link Logger}s
     */
     ...
     * Set the root logger.
     * <p>
     * Note that if any classes have statically obtained their logger instance
     ...
     * call {@link #getLogger(String)} on it and construct a {@link LoggerLog}
     * instance as this Log's Logger, so that logging is delegated to the parent
     * Log.
     * <p>
     * This should be used if a webapp is using Log, but wishes the logging to
     * be directed to the containers log.
     * <p>
