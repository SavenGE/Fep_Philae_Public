 ...
 * <p>
 * Calls to {@link #execute(Runnable)} on a {@link ReservedThreadExecutor} will
 * either succeed with a Thread immediately being assigned the Runnable task, or
 * fail if no Thread is available.
 * <p>
 * Threads are reserved lazily, with a new reserved thread being allocated from
 * a wrapped {@link Executor} when an execution fails. If the
 * {@link #setIdleTimeout(long, TimeUnit)} is set to non zero (default 1
 * minute), then the reserved thread pool will shrink by 1 thread whenever it
...
public class ReservedThreadExecutor extends AbstractLifeCycle implements TryExecutor {
     ...
     * @param capacity The number of threads to preallocate. If less than 0 then
     * capacity is calculated based on a heuristic from the number of available
     * processors and thread pool size.
     */
     ...
     * capacity is calculated based on a heuristic from the number of available
     * processors and thread pool size.
     * @return the number of reserved threads that would be used by a
     * ReservedThreadExecutor constructed with these arguments.
     */
     ...
     * @return True iff a reserved thread was available and has been assigned
     * the task to run.
     */
    @Override
    ...
    private class ReservedThread implements Runnable {
        ...
        public void run() {
            while (isRunning()) {
                ...
                if (task == STOP) // return on STOP poison pill
                {
                    break;
                }
