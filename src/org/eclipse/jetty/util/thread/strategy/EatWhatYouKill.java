 ...
 * when a thread produces a task it can immediately run the task and let the
 * pending producer thread take over production. When operating in this way, the
 * sub-strategy is called Execute Produce Consume (EPC).</p>
 * <p>
 ...
 * task has completed. When operating in this pattern, the sub-strategy is
 * called ProduceConsume (PC).</p>
 * <p>
 * If there is no pending producer thread available and if the task has not
 * indicated it is non-blocking, then this strategy will dispatch the execution
 * of the task and immediately continue production. When operating in this
 * pattern, the sub-strategy is called ProduceExecuteConsume (PEC).</p>
 */
